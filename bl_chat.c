...
int BotSameTeam(bot_state_t *bs, int client)
{
	...
	/* LMCTF keeps the team on client->ctf.teamnum, not resp.ctf_team, and
	 * spells "no team" CTF_TEAM_UNDEFINED. */
	return (bot_ent->client->ctf.teamnum != CTF_TEAM_UNDEFINED &&
	        bot_ent->client->ctf.teamnum == other_ent->client->ctf.teamnum);
}
