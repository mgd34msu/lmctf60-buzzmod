...
for ln in lines:
    m = re.match(r'SG (\S+?)\[SG\]: role=(\d) seed=\d+ goal=(-?\d+) spd=(\d+) org=\((-?\d+) (-?\d+)', ln)
    if not m: continue
    name, role, goal, spd = m.group(1), int(m.group(2)), int(m.group(3)), int(m.group(4))
    if role == 0 and goal > 0:
        goals[name] = min(goals.get(name, 1 << 30), goal)
    if role == 1:
        patrol_pos.add((int(m.group(5)) // 100, int(m.group(6)) // 100))
