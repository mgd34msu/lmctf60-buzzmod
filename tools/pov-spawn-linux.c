...
static void child_run(const struct pov_image*i,char*const av[],char*const ep[],const struct pov_stdio*m,int ef,pid_t parent){int e;sigset_t empty;sigemptyset(&empty);if(prctl(PR_SET_PDEATHSIG,SIGKILL)<0||getppid()!=parent||sigprocmask(SIG_SETMASK,&empty,NULL)<0||mapfd(m->input,0)<0||mapfd(m->output,1)<0||mapfd(m->error,2)<0||(m->cwd>=0&&fchdir(m->cwd)<0)||syscall(SYS_close_range,3U,UINT_MAX,CLOSE_RANGE_CLOEXEC)<0)goto fail;execveat(i->fd,"",av,ep,AT_EMPTY_PATH);fail:e=errno;(void)write(ef,&e,sizeof(e));_exit(127);}
