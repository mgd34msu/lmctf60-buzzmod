...
# ----------------------------------------------------------------- MODULE
# NOTES (limitations a judge should know before trusting a routes sheet):
#
# 1. The node graph is a quantization of the map's rune seed cloud, which is
#    the bot navigation build's model of the walkable floor -- not ground
#    truth geometry. Where the seed cloud is thin (high ledges, water, hook
#    routes over open space) the graph is thin too, and traffic there lands on
#    whichever node happens to be nearest. That is exactly what panel 6 is for:
#    it measures how much of a player's time the graph does NOT explain,
#    rather than pretending the graph explains all of it.
#
# 2. Node assignment is planar (x/y). Two floors of the same stairwell share a
#    node. This is deliberate: separating them would fragment one lane into
#    several nodes and make the transition matrix read as improvisation when
#    the player simply went up some stairs.
#
# 3. The dwell filter (ROUTE_DWELL_MIN_S) is the one place a coverage
#    difference can still reach this sheet. Dwell is counted in observed
#    samples, so a player who is only intermittently sampled accumulates
#    observed dwell more slowly and can have a real visit filtered out as
#    chatter. The effect shrinks the sequence rather than changing its shape,
#    and ROUTE_MIN_TRANSITIONS drops the players where it has bitten hardest,
#    but it is not zero. It is the residual this rung declares.
#
# 4. Entropy is estimated from a finite sequence and is biased LOW at small
#    sample sizes. ROUTE_MIN_TRANSITIONS = 60 is the floor below which no bar
#    is drawn at all; between 60 and a few hundred transitions the estimate is
#    still noisy. Compare bars within a sheet with more confidence than across
#    two sheets whose coverage differs.
#
# 5. WHICH PANELS ARE MEASURING BEHAVIOUR AND WHICH ARE MEASURING COVERAGE.
#    Measured, not assumed: Stage A was re-run with the pov-parity radius at
#    800u and 1000u either side of the calibrated 900u (the design's standard
#    "is this separation behavioural or is it PVS" test). A scalar whose AUC
#    moves under that perturbation is reading how much of the match was
#    sampled, not how the players moved. On mactf06, 4 human vs 13 bot demos:
#
#      scalar                   AUC @800  @900  @1000   verdict
#      offgraph_fraction           0.000  0.000  0.000  stable  -> behavioural
#      revisit_spike2_mass         0.942  0.923  0.923  stable  -> behavioural
#      occupancy_kl_bits           0.038  0.038  0.019  stable  -> behavioural
#      mean_route_entropy_bits     0.077  0.154  0.288  swings 0.14 -> coverage
#      edge_density                0.231  0.308  0.067  swings 0.24 -> coverage
#
#    So panels 4, 5 and 6 are the ones reading behaviour; panels 1, 2 and 3
#    must be reported to judges as supporting context, not as evidence, until
#    the parity model stops moving them. The mechanism is visible in the
#    sidecar: a wider parity radius keeps more samples, longer node sequences
#    clear ROUTE_MIN_TRANSITIONS, and both the entropy estimate and the count
#    of edges above EDGE_P_MIN rise with sequence length. That is a sample-size
#    effect on an estimator, not a route-choice difference.
#
# 6. AND WHICH OF THOSE SURVIVE A CHANGE OF MAP. Stage A was replicated on a
#    second map (lmctf22, 4 human vs 13 bot). Separability, mactf06 -> lmctf22:
#
#      occupancy_kl_bits        0.962 -> 1.000   replicates, same direction
#      offgraph_fraction        1.000 -> 0.577   does NOT replicate
#      revisit_spike2_mass      0.923 -> 0.615   does not replicate, and the
#                                                direction inverts
#
#    Only panel 5 (map-occupancy divergence -- humans spread their time over
#    more of the map than the bots do, 0.56 vs 0.40 bits on mactf06 and 0.81
#    vs 0.39 on lmctf22) is strong on both maps, in the same direction, and
#    stable under the parity-radius perturbation. It is the one result this
#    instrument currently earns.
#
#    The off-graph collapse is explained, not mysterious: on lmctf22 BOTH arms
#    read ~0.0005, i.e. that map's seed cloud covers essentially all of its
#    walkable floor, so there is no off-graph space for anyone to occupy. The
#    panel is not broken there -- it is correctly reporting that the map has
#    no unmapped geometry to hug. That makes off-graph a map-conditional tell,
#    and a judge set built only on mactf06 would over-credit it. Spread the
#    sets across maps, exactly as the design's five-map pool intends.
#
#    The revisit inversion is the more serious one and echoes what judge set
#    #2 found on rung 1's corridor needle: a discriminator that reverses sign
#    between maps is not measuring what it is named after. Do not build a bot
#    change on panel 4 until it has replicated on a third map.
#
# 7. Every panel pools or normalizes away the thing rung 1 got burned on:
#    there is no count, no duration and no roster size anywhere on the PNG.
#    What remains readable is the SHAPE of the traversal -- which is the
#    quantity this rung exists to show, and also the only quantity here that a
#    bot could move by playing better rather than by playing worse (the
#    design's Rule 21 exposure for this rung is nil: route variety, map
#    coverage and not hugging geometry are all competence improvements).
