...
maps/<map>.hmn: 16-byte header then one uint8 per link -- the link's
...
def main():
    ...
    for mapname in sys.argv[3:]:
        ...
        tiers = bytes(
            ...
            for c in counts)
        out = os.path.join(os.path.dirname(rp), f'{mapname}.hmn')
        with open(out, 'wb') as f:
            f.write(struct.pack('<4i', HMN_MAGIC, 1, nl, 0))
            f.write(tiers)
