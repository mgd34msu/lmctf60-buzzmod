...
| Area | Current state |
|---|---|
| Architectural state | The static-model contract, complete BSP reader, authoritative BSP collision and Pmove adapter, positive-volume standing/crouching BSP configuration space, audited host-uniform support/water/air/void partitions, complete sky-aware hook-surface catalogs, host-parity static entity and mechanism semantics, BSP-bound point visibility and occlusion queries, an independent BSP completeness proof, independently accepted ground and water capability foundations without configurable workload caps, canonical artifact codec, fail-closed artifact loader, atomic joint artifact publication, independent C and Python artifact readers, an independently reviewed semantic-acceptance foundation, directional destination-field solver with generational cache, typed strategy compiler/reducer, transactional phase-space belief reducer, authenticated perception-evidence adapter, identity-bound host-parity weapon-effect profiles, and an independently accepted static weapon-affordance foundation are integrated on `slipgate`. The hook-visibility architecture is now synthesized in `docs/hook-visibility-relation.md`: exact symbolic host-law proof with BSP-guided predicate discovery, reduced runtime relations, and a non-enumerative feasibility gate precede hook/air construction. Mechanism acceptance, hook-visibility feasibility and hook/air construction, shared cost-kernel integration, the independent complete-model proof provider, semantic publication wiring, and production navigation, strategy, perception, and combat host hookups remain open. |
