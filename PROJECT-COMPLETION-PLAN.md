...
| Map | Current result |
| --- | --- |
| `lmctf01` | A fresh integrated-source rerun again proves 1,776 seeds/16,816 links, then all prune: 999 red-only, 425 blue-only, 0 shared, and 352 neither. The sixteen closest opposing pairs are 252--260 units apart. Exact replay in both directions proves zero RUN, JUMP, DROP, HOOK, SWIM, or rocket-jump traversals. The only two cross-partition SWIM boundaries run one way from the red-only partition into neutral geometry; their reverse traversals fail and adding either cannot close the graph. The BSP's only external button-door pair is causal. Replacing the old boundary-stop command with a generic brush-center inward contact turns 0/165 into 39/165 exact stock `button_touch` proofs with stable authenticated anchors. Ordinary door-face staging is refuted: the 16 nearest graph seeds are about 384 units above the leaf, and 4,224 authenticated open-pose probes around both faces derive zero stable ground points. The existing automatic-trigger compound DROP owner is also noncausal because it does not schedule this external button. The button-open static-anchor DROP inventory finds 15 valid walkoffs; every replay reaches supported terrain but stops at `SHALLOW_WATER_CONTACT` while steering toward the authenticated deep-water destination, with no canonical seed within 198--211 units of the shallow contact. The current diagnostic keeps the serialized DROP law unchanged and removes only that early refusal to determine whether the physical replay reaches the deep-water destination. |
...
| `lmctf25` | The old timeout is gone. Current immutable generation proves 2,092 seeds/20,601 links, then all prune: 547 red-only, 536 blue-only, 0 shared, and 1,009 neither. The closest opposing components remain separated by an 835-unit vertical cut. |
...
| `lmctf30` | Current immutable generation proves 746 seeds. The causal authored mechanisms are two 418-unit vertical toggle carriers, each with separate solid top and bottom buttons. The generic two-entry toggle-carrier replay authenticates all four directed transactions and adds exactly four `RL_LIFT` links, raising the graph from 3,798 to 3,802 links and changing the partitions from 248 red-only/250 blue-only/0 shared/248 neither to 258 red-only/260 blue-only/0 shared/228 neither. The apparent 815-unit upper cut was reduced to two symmetric stock-platform boundaries. Correcting the local staging radius from 16 units to Quake II's authoritative 18-unit step height makes every nearby bottom source replay successfully, but the only top seed inside the existing 320-unit lift-egress envelope has no outgoing continuation and is correctly rejected. A diagnostic global 320-unit RUN admission proves both lifts and adds 2,387 unrelated links, yet still leaves 0 shared seeds, so it is noncausal and reverted. The current exact cut is a pair of symmetric deep-water side components approaching neutral water through 100.519-unit vertical pairs; SWIM rejects both directions, and the replay failure reason is being isolated without widening global movement admission. |
