...
| Map | Current result |
| --- | --- |
| `lmctf01` | A fresh integrated-source rerun again proves 1,776 seeds/16,816 links, then all prune: 999 red-only, 425 blue-only, 0 shared, and 352 neither. The sixteen closest opposing pairs are 252--260 units apart. Exact replay in both directions proves zero RUN, JUMP, DROP, HOOK, SWIM, or rocket-jump traversals. The only two cross-partition SWIM boundaries run one way from the red-only partition into neutral geometry; their reverse traversals fail and adding either cannot close the graph. The BSP's only external button-door pair is now the exact candidate. Across five authenticated wait points, 165 current-bound approach calls produce no proof, while an exhaustive stable dry seed-pair scan finds no pair whose segment crosses the target door sweep. This is a seed/staging omission at both ends, not the already refuted direct-controller vertical bound; the current probe is deriving canonical graph states from physical button contact and open-door landing observations. |
...
| `lmctf30` | Current immutable generation proves 746 seeds. The causal authored mechanisms are two 418-unit vertical toggle carriers, each with separate solid top and bottom buttons. The generic two-entry toggle-carrier replay now authenticates all four directed transactions: both lower underwater approaches, carrier boarding/carry, top egress, both upper dry approaches, post-callback boarding, settle, and lower egress. It adds exactly four `RL_LIFT` links, raising the graph from 3,798 to 3,802 links and changing the partitions from 248 red-only/250 blue-only/0 shared/248 neither to 258 red-only/260 blue-only/0 shared/228 neither. The remaining blocker is a distinct 815-unit upper-level objective cut with no boundary link; exact RUN, JUMP, DROP, HOOK, and rocket-jump replay fails in both directions. The carrier unit is being stripped of diagnostics and reduced independently while that new cut is investigated. |
