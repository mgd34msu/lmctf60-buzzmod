...
| `lmctf12` | Fresh current source has 1,972 seeds after sparse-water integration: 939 red-only, 921 blue-only, 0 shared, and 112 neither. Its 26-SCC condensation has the red and blue roots in separate sink SCCs with no outgoing links. Native RUN/JUMP/DROP/HOOK/rocket-jump replay rejects the 16 nearest 240-unit cross-SCC pairs; crouch also proves zero. Exact SWIM replay proves the nearby ramp handoff in both directions, but adding it only absorbs an isolated neutral SCC into the red sink; the old all-pair water graph also fails, so the sparse-water omission is noncausal. DROP replay over 449 red-sink to neutral-center pairs and all 16 serialized headings finds 353 geometric lips but zero exact arrivals. Exact upper-room RUN/JUMP/HOOK replay also proves zero. BSP collision proves the apparent bridge decks are sealed behind a world wall through z=672, and delayed jumps pin at that wall. All six teleport pads have two stable staging seeds. Both pad-local replay and broader real-time approaches authenticate every entry, including the two back-pad 136-unit approaches, but expose only a 25-seed shared back-room component. Its nearest objective-side exits are red 115->969 at 201.435 units and blue 117->965 at 208 units; exact RUN, JUMP, DROP, HOOK, and rocket-jump all refute on the nearest 64 pairs per side. Raising the hook frontier to 32,768 calls adds 28 links but still yields zero shared seeds, so hook budget is noncausal. The current bounded scan is exhausting every side-to-shared family within 512 units to locate or conclusively refute the geometric egress. |
...
| `lmctf30` | Current immutable generation proves 746 seeds. The causal authored mechanisms are two 418-unit vertical toggle carriers, each with separate solid top and bottom buttons. The generic two-entry toggle-carrier replay authenticates all four directed transactions and adds exactly four `RL_LIFT` links, raising the graph from 3,798 to 3,802 links and changing the partitions from 248 red-only/250 blue-only/0 shared/248 neither to 258 red-only/260 blue-only/0 shared/228 neither. The apparent 815-unit upper cut has been reduced to two symmetric 64.792-unit boundaries around stock `func_plat` entities. Bottom staging was rejected solely because `Gen_MechanismSeedNear` used 16 units while authoritative Quake II Pmove step height is 18; using 18 makes every nearby stable source replay successfully in 175--350 ms. Each raised platform then has one nearby stable top seed with no outgoing link, while an exact bidirectional RUN proof reaches a connected same-level seed 229--231 units away. The global base admission remains unchanged; the current test lets only the authenticated stock-lift egress search its existing bounded 320-unit planar envelope. |
