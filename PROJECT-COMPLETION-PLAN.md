...
| Area | Current state |
|---|---|
| Architectural state | The static-model contract, complete BSP reader, authoritative BSP collision and Pmove adapter, positive-volume standing/crouching BSP configuration space, audited host-uniform support/water/air/void partitions, complete sky-aware hook-surface catalogs, canonical artifact codec, fail-closed artifact loader, atomic joint artifact publication, independent C and Python artifact readers, directional destination-field solver with generational cache, typed strategy compiler/reducer, transactional phase-space belief reducer, authenticated perception-evidence adapter, and identity-bound host-parity weapon-effect profiles are integrated on `slipgate`. Independent correction passes closed host-parity, transactional-decode, capability-domain, terminal-physics, moving-target freshness, API-version, exact-sidecar-identity, publication-restart, reader-scaling, large-file-portability, strategy-authority, fixed-point-capacity, evidence-authority, belief-scaling, plane-scale, zero-volume host-rounding, teammate-location-laundering, perception-buffer, weapon-law, weapon-query-boundary, source-mesh, support-partition, source-leaf, hook-rounding, and semantic-audit defects. Static entity/mechanism attachment, movement construction, and production navigation, strategy, perception, and combat host hookups remain open. |
