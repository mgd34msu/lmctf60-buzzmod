...
| `tw2ctf4` | Current immutable generation proves 2,066 seeds/31,850 links, then all prune: 596 red-only, 606 blue-only, 0 shared, and 864 neither. Exact condensation puts the roots in separate 555- and 562-seed SCCs, with a third isolated 414-seed neutral SCC. Four single-member central shoot-door teams prove eight bidirectional links across SCC 42 to SCCs 34 and 25. Complete team binding does not change either two-member base result: 125 candidates per direction still time out cleanly at the threshold, with no contamination. Holding crouch also proves zero. Direct standing and crouched hull traces across the first admitted pair in each direction are identifying whether the remaining blocker is the opened brush, world geometry, or steering before any jump controller is attempted. |
| `xmap02` | Current immutable generation proves 1,462 seeds/21,679 links, including two teleport links, then all prune: 698 red-only, 682 blue-only, 0 shared, and 82 neither. The closest opposing seeds are 345 units apart; the other four teleporter pads still lack staging, but causality remains unproved. |
