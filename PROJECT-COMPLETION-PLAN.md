...
| Area | Current state |
|---|---|
| Architectural state | The static-model contract, complete BSP reader, authoritative BSP collision and Pmove adapter, positive-volume standing/crouching BSP configuration space, audited host-uniform support/water/air/void partitions, complete sky-aware hook-surface catalogs, host-parity static entity and mechanism semantics, BSP-bound point visibility and occlusion queries, an independent BSP completeness proof, and independently accepted ground, water, mechanism, and static weapon-affordance capability foundations without configurable workload caps are integrated on `slipgate`. Canonical artifact codec, fail-closed artifact loader, atomic joint artifact publication, independent C and Python artifact readers, semantic-acceptance foundation, directional destination-field solver with generational cache, typed strategy compiler/reducer, transactional phase-space belief reducer, authenticated perception-evidence adapter, and identity-bound host-parity weapon-effect profiles are also integrated. The hook-visibility architecture is synthesized in `docs/hook-visibility-relation.md`: exact symbolic host-law proof with BSP-guided predicate discovery, reduced runtime relations, and a non-enumerative feasibility gate precede hook/air construction. Hook-visibility feasibility and hook/air construction, shared cost-kernel integration, the independent complete-model proof provider, semantic publication wiring, and production navigation, strategy, perception, and combat host hookups remain open. |
...
| ART-4 | Maintain independent GNU C, Make C, and Python readers, lint, semantic validation, exact-bound sidecars/SNAG where still applicable, and fresh-process cold load. | Standalone C and Python readers and a production-loader probe pass a fresh-process differential suite without importing generator logic. The reviewed semantic-acceptance foundation compares all 13 sections against an opaque complete-model proof catalog and rejects proof-storage aliasing before decode or publication. The real complete-model proof provider, production publication caller, exact-bound sidecar/SNAG handling, and installed cold load remain **review/reshape** work. | Differential malformed corpus and valid corpus produce agreement; no reader imports generation logic as its oracle. |
| ART-5 | Review whether SNAG remains meaningful under the new field. Retain only bounded runtime cost annotations; delete any geometry-repair or obsolete route-only role. | Review concluded that the seed/link SNAG has no valid v2 consumer. `SNAG-DECISION.md` requires deletion of its runtime, producers, controller state, and release transport. Runtime ownership is deleted and verified. Producer, controller, release, iteration, POV, build, and documentation ownership remain under isolated review. | No legacy SNAG reference remains. Any future learned-cost sidecar is a new stable-cell or capability-kernel contract that cannot alter geometry or connectivity. |
...
- [ ] Pass the non-enumerative hook-visibility feasibility gate, then build the
  complete relation catalog and separate bolt, body, pull, release, coast, and
  relaunch physics. Keep human hook code isolated.
