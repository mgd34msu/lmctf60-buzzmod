...
| `tw2ctf2` | The exact four-link batch closes the real graph: one grounded waypoint RUN plus one exact HOOK per side yields 1,922 seeds reachable from both roots and writes a 1,983-seed/33,586-link artifact. Current GNU/Make C readers and Python accept it; the failure came from stale reader binaries in the diagnostic snapshot. A fully current frozen snapshot rerun is next. |
...
| `xmap04` | 1,094 seeds all prune. Exact center boarding proves one honest reverse RL_LIFT on the linked 768-unit platform, but the objective partitions remain unchanged, so that descent is not causal and is not integration-ready. The active diagnostic is the separate missing bottom staging for the tenth 768-unit platform; failure to close there refutes the lift hypothesis. |
