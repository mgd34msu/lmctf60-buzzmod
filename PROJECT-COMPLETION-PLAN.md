...
| `lmctf12` | Fresh current source has 1,972 seeds after sparse-water integration: 939 red-only, 921 blue-only, 0 shared, and 112 neither. Its 26-SCC condensation has the red and blue roots in separate sink SCCs with no outgoing links. Native RUN/JUMP/DROP/HOOK/rocket-jump replay rejects the 16 nearest 240-unit cross-SCC pairs; crouch also proves zero. Exact SWIM replay proves the nearby ramp handoff in both directions: seed 1607 -> 1573 in 600 ms and 1573 -> 1607 in 700 ms, with a clear hull trace. The fixed-offset water discovery cannot nominate that 24/112/56-unit pair, but adding it only absorbs an isolated neutral SCC into the red sink; the old all-pair water graph also fails, so this omission is not the lmctf12 blocker. DROP replay over 449 red-sink to neutral-center pairs and all 16 serialized headings finds 353 geometric lips but zero exact arrivals, refuting the existing DROP family. Live traversal identifies the physical lead as a submerged corner route: the direct approach pins at x=-360, while moving south along the obstruction reaches the center section. The exact turn and eastward handoff are now being isolated. |
...
| `lmctf30` | Current immutable generation proves 746 seeds/3,798 links, then all prune: 248 red-only, 250 blue-only, 0 shared, and 248 neither. The causal authored mechanisms are two 418-unit vertical toggle carriers, each with separate solid top and bottom buttons. The current single-entry auto-return platform catalog excludes this bidirectional topology. Both stock platforms have valid rest poses but no static staging; widened unchanged approach replay and direct DROP-to-platform replay prove zero, so they are noncausal. This repair follows the overlapping `lmctf06` platform work. |
...
| `tw2ctf4` | Current immutable generation proves 2,066 seeds/31,850 links, then all prune: 596 red-only, 606 blue-only, 0 shared, and 864 neither. Exact condensation puts the roots in separate 555- and 562-seed SCCs, with a third isolated 414-seed neutral SCC. Four single-member central shoot-door teams prove eight bidirectional links across SCC 42 to SCCs 34 and 25. The two-member base doors leave a lowered 28-unit leaf in the route; unconditional jumping was premature and reached it with jump still held. Exact gap-timed shoot-door replay now proves all four base legs, including both reverse jumps to canonical world-supported seeds. Those four links plus the eight central links close both objective directions. The generic shoot-door action and runtime contract is being reduced from this exact witness set; no diagnostic code will publish. |
...
| `xmap18` | Current immutable generation proves 2,417 seeds/28,556 links, then all prune: 223 red-only, 219 blue-only, 0 shared, and 1,975 neither. Exact condensation has 48 SCCs. The objective regions are SCC 11 with 210 seeds and SCC 42 with 205; the main red and blue geometry is stranded in neutral SCCs 44 with 595 seeds and 47 with 584, while center SCC 45 has 198. All five authored teleporter approach seeds exist within 12 to 83 units of their pads but remain in tiny or isolated SCCs. The missing route is upstream of teleport publication. Fixed-offset water discovery is known to omit some replay-proved pairs, but that omission is noncausal on `lmctf12`; each xmap18 pad-staging water chain therefore needs its own exact graph-effect replay before the discovery rule changes. |
