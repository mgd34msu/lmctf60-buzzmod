...
Autonomous escorts now finish at the source of the selected carrier or moving-
formation field rather than reverting to the capture stand at their local
minimum. Exact human cover targets still outrank that field, and SCOOP retains
its separate dropped-flag terminal law.
Cut-rope and rocket-jump pre-turn now reads the active movement route field.
...
Full-roster standoff reconciliation now counts recovery coverage only from a
non-carrier body that can actually receive the RECOVER duty. A carrier's finite
...
The carrier-support component uses that same effective mission. A concrete
...
- [ ] Bot improvement backlog measured, resolved, and consumer-proven across
      ...
      ESCORT terminal fallback follows its selected carrier/formation field;
      exact human cover targets and relay SCOOP remain separately authoritative.
      ...
      reaction/settle state. Opponent range terms are frozen at the last
      visible sighting; hidden quad, plasma-mode, or other live state changes
      cannot steer the bot's subsequent corner behavior.
      ...
      The default human-derived stolen-flag exit draw uses the same immutable
