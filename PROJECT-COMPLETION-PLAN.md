...
| Map | Current result |
| --- | --- |
| `lmctf01` | A fresh integrated-source rerun again proves 1,776 seeds/16,816 links, then all prune: 999 red-only, 425 blue-only, 0 shared, and 352 neither. The sixteen closest opposing pairs are 252--260 units apart. Exact replay in both directions proves zero RUN, JUMP, DROP, HOOK, SWIM, or rocket-jump traversals. The only two cross-partition SWIM boundaries run one way from the red-only partition into neutral geometry; their reverse traversals fail and adding either cannot close the graph. The BSP's only external button-door pair is now the exact candidate: generation accepts five wait points at the remote button, but the direct controller schedules zero approach and zero egress trials. Expanding only its vertical egress envelope still schedules none, proving this requires a staged button-to-door route rather than a broader direct bound. |
...
| `lmctf30` | Current immutable generation proves 746 seeds/3,798 links, then all prune: 248 red-only, 250 blue-only, 0 shared, and 248 neither. The causal authored mechanisms are two 418-unit vertical toggle carriers, each with separate solid top and bottom buttons. Both stock platforms have valid rest poses but no static staging; widened unchanged approach replay and direct DROP-to-platform replay prove zero, so they are noncausal. The generic two-entry toggle-carrier implementation passes focused catalog, plan, execution, runtime-currentness, production-linkage, and both-reader tests. Real replay now authenticates both lower underwater approaches and carrier support but still lacks top egress; both upper dry entries contact their buttons but fail the straight carrier-support stage. |
