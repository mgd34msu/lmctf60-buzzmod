...
| `lmctf06` | Fresh current source has 1,437 seeds/12,528 links, all prune. The generic catalog and approach repair proves four authenticated button-to-vertical-carrier lift links and raises red reach from 206 to 477 seeds; blue reaches 676, with no shared seed. Exact button contact, cabin-floor rest, ascent, and top egress all prove. The remaining central cut is not slick movement or lift reversal: zero-command replay at seeds 976/977 finds world-only contamination from a START_ON/TOGGLE `func_wall`. The authored closure is button 447 through relay 449 to switch wall 443 off, followed by relay 450 switching it on four seconds later. Representation of that exact timed toggle is being checked before any admission is published. |
| `lmctf07` | PASS after the accepted train/button repair. |
| `lmctf12` | Fresh current source has 1,972 seeds after sparse-water integration: 939 red-only, 921 blue-only, 0 shared, and 112 neither. Its 26-SCC condensation has the red and blue roots in separate sink SCCs with no outgoing links. Native RUN/JUMP/DROP/HOOK/rocket-jump replay rejects the 16 nearest 240-unit cross-SCC pairs. Exact 320 and 400 unit-per-second running jumps are stopped symmetrically by world faces. All six teleporter approaches prove only after their production scheduling point, and a late admission pass publishes zero, refuting teleport ordering. Crouch replay also proves zero standing-blocked/duck-clear sweeps. The symmetric side-well-to-center cut is now being traced as a compound obstacle route, starting with the authoritative RUN/JUMP trajectory from red seed 1127 to neutral seed 1122. |
...
| `tw2ctf4` | Current immutable generation proves 2,066 seeds/31,850 links, then all prune: 596 red-only, 606 blue-only, 0 shared, and 864 neither. Exact condensation puts the roots in separate 555- and 562-seed SCCs, with a third isolated 414-seed neutral SCC. Fair unchanged RUN replay proves zero across both objective cuts. All six shoot-activated door-team cuts are now exact: four single-member central teams prove eight bidirectional links across SCC 42 to SCCs 34 and 25. The two two-member base teams admit 125 candidates per direction but the single-mover train oracle proves zero because it does not bind the slave sweep. Those base cuts are being replayed through the existing complete door-team binding path; the incidental central links alone are not publishable. |
...
| `xmap18` | Current immutable generation proves 2,417 seeds/28,556 links, then all prune: 223 red-only, 219 blue-only, 0 shared, and 1,975 neither. The closest opposing components are 5,442 units apart with no initial boundary link; teleporter admission was exactly replayed and refuted. |
