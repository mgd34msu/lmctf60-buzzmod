...
| Area | Current state |
|---|---|
| Architectural state | Integrated on `slipgate`: frozen contracts; complete BSP, collision, hull configuration, semantic, visibility, hook-surface, and completeness foundations; accepted ground, water, mechanism, and weapon foundations; artifact codec, loader, atomic publication, three independent readers, and semantic acceptance; portable runtime-map preflight; destination fields; typed strategy; sparse beliefs with exact life identity; weapon profiles; passive human traces; and hook-visibility publication. Hook/air construction, shared costs, complete-model proof, semantic publication, and production navigation, strategy, perception, combat, and learning hookups remain open. |
