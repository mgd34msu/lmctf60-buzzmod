...
| `lmctf06` | The integrated generic catalog and approach repair proves four authenticated button-to-vertical-carrier `RL_LIFT` links. The real rerun has 1,437 seeds/12,965 links and raises red reach from 206 to 477 seeds; blue reaches 676, with no shared seed. Exact button contact, cabin-floor rest, ascent, and top egress all prove. The remaining central cut is not slick movement or lift reversal: zero-command replay at seeds 976/977 finds world-only contamination from a START_ON/TOGGLE `func_wall`. Temporarily opening that wall produces 1,496 seeds/16,674 links and 143 shared seeds, proving the button 447 -> relay 449 -> wall 443 off -> delayed relay 450 -> wall on transaction is causal. Two exact reverse HOOK repairs then increase the shared set to 760 but leave 592 red-only, so the wall transaction is necessary but not sufficient. Its exact representation is being built without publishing the global-open diagnostic. |
