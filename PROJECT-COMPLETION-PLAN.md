...
| Area | Current state |
|---|---|
| Architectural state | The static-model contract, complete BSP reader, authoritative BSP collision and Pmove adapter, positive-volume standing/crouching BSP configuration space, audited host-uniform support/water/air/void partitions, complete sky-aware hook-surface catalogs, host-parity static entity and mechanism semantics, BSP-bound static visibility and occlusion queries, canonical artifact codec, fail-closed artifact loader, atomic joint artifact publication, independent C and Python artifact readers, directional destination-field solver with generational cache, typed strategy compiler/reducer, transactional phase-space belief reducer, authenticated perception-evidence adapter, and identity-bound host-parity weapon-effect profiles are integrated on `slipgate`. Independent correction passes closed host-parity, transactional-decode, capability-domain, terminal-physics, moving-target freshness, API-version, exact-sidecar-identity, publication-restart, reader-scaling, large-file-portability, strategy-authority, fixed-point-capacity, evidence-authority, belief-scaling, plane-scale, zero-volume host-rounding, teammate-location-laundering, perception-buffer, weapon-law, weapon-query-boundary, source-mesh, support-partition, source-leaf, hook-rounding, entity-parser, target-topology, static-visibility, and semantic-audit defects. Movement construction and production navigation, strategy, perception, and combat host hookups remain open. |
...
| ART-4 | Maintain independent GNU C, Make C, and Python readers, lint, semantic validation, exact-bound sidecars/SNAG where still applicable, and fresh-process cold load. | Standalone C and Python readers and a production-loader probe now pass a fresh-process differential suite without importing generator logic. Their aggregate gate proves logarithmic reference lookup and portable 64-bit file sizing. Exact-bound sidecar/SNAG and installed cold-load integration remain **review/reshape** work. | Differential malformed corpus and valid corpus produce agreement; no reader imports generation logic as its oracle. |
