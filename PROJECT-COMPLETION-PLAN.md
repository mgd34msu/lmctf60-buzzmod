...
`lmctf25`, `tomb05`, `xmap13`, `xmap18`, and `xmap26`.
...
| Map | Current result |
| --- | --- |
| `lmctf01` | A fresh integrated-source rerun again proves 1,776 seeds/16,816 links, then all prune: 999 red-only, 425 blue-only, 0 shared, and 352 neither. The sixteen closest opposing pairs are 252--260 units apart. Exact replay in both directions proves zero RUN, JUMP, DROP, HOOK, SWIM, or rocket-jump traversals. The only two cross-partition SWIM boundaries run one way from the red-only partition into neutral geometry; their reverse traversals fail and adding either cannot close the graph. Replacing the old boundary-stop command with a generic brush-center inward contact turns 0/165 into 39/165 exact stock `button_touch` proofs with stable authenticated anchors, but the external button-door is not the objective cut. Ordinary door-face staging produces zero stable ground points across 4,224 authenticated open-pose probes. The button-open DROP investigation reaches five stable waterlevel-three states, but all remain in the same objective-side component. Exact continuation replays from those states add no missing objective bit. Across every authenticated target-sweep incident pair, zero stable/outgoing endpoints in different SCCs cross within the existing 768-unit door envelope. All eight lift links also remain inside their existing side or neutral SCCs. The closest symmetric side-to-neutral cuts are 58.293 units across a permanent world divider: point line-of-sight is clear, but a standing player hull immediately hits world geometry, stock movement stops short, and exact replay of RUN, JUMP, DROP, HOOK, rocket jump, and SWIM proves zero crossings in all four directed pairs. The remaining red-to-neutral SWIM edges are one-way and noncausal. Automated source inventory is exhausted with no production change retained; `lmctf01` is reserved for an end-of-plan authoritative human traversal. |
| `lmctf06` | The integrated generic catalog and approach repair proves four authenticated button-to-vertical-carrier `RL_LIFT` links. The real rerun has 1,437 seeds/12,965 links and raises red reach from 206 to 477 seeds; blue reaches 676, with no shared seed. Exact button contact, cabin-floor rest, ascent, and top egress all prove. The remaining central cut is not slick movement or lift reversal: zero-command replay at seeds 976/977 finds world-only contamination from a START_ON/TOGGLE `func_wall`. Temporarily opening that wall against current source produces 1,496 seeds and 16,674 pre-repair links, proving the button 447 -> relay 449 -> wall 443 off -> delayed relay 450 -> wall on transaction is causal. The current fixed-point inverse repair then adds four exact reverse HOOK links and closes all 1,352 objective-reachable seeds. The only remaining lmctf06 implementation is therefore an authenticated runtime representation of that temporary button/relay/forcefield transaction; the global-open diagnostic is not publishable. |
| `lmctf07` | PASS after the accepted train/button repair. |
| `lmctf12` | Fresh current source has 1,972 seeds after sparse-water integration: 939 red-only, 921 blue-only, 0 shared, and 112 neither. Its 26-SCC condensation has the red and blue roots in separate sink SCCs with no outgoing links. Exact SWIM replay proves the nearby ramp handoff in both directions, but adding it only absorbs an isolated neutral SCC into the red sink; the old all-pair water graph also fails, so the sparse-water omission is noncausal. DROP replay over 449 red-sink to neutral-center pairs and all 16 serialized headings finds 353 geometric lips but zero exact arrivals. BSP collision proves the apparent bridge decks are sealed behind a world wall through z=672, and delayed jumps pin there. All six teleport pads have two stable staging seeds. Pad-local and broader real-time approaches authenticate every entry and expose a real 25-seed shared back-room component, but the complete existing-controller family through the 768-unit hook domain proves no objective-side egress: red 64 nearest plus 1,148 extended pairs, and blue 64 plus 1,212 extended pairs, all yield zero RUN, JUMP, DROP, HOOK, or rocket-jump proofs. Thirty-one reverse shore-drop trials also yield zero; a 32,768-call hook frontier still produces zero shared seeds. The exact reverse witnesses 1509->1821 HOOK and 82->84 rocket-jump are real but noncausal. This inventory is exhausted with no generic proof-backed source change; the diagnostic worktree is clean and `lmctf12` is reserved for an end-of-plan authoritative human traversal. |
| `lmctf15` | Current immutable generation proves 1,953 seeds/19,785 links, then all prune: each flag remains isolated in a separate ten-seed component and no seed is shared. Each mirrored exterior button targets a two-leaf rotating flag door and delayed relays: after one second the relays disable eight START_ON lasers, then restore them with the door after ten seconds. Shortest-path reduction proves that four directed exterior-to-flag and flag-to-exterior transactions are sufficient to close 1,800 seeds from both roots. Those edges are not publishable as ordinary RUN or BUTTON_DOOR links: the existing controller authenticates only a button whose complete target fanout is one canonical door team, does not model the laser relays, and cannot retain the timed lease across flag pickup for immediate egress. The correct source repair is a generic authenticated timed flag-vault controller, not relaxed target validation or speculative movement links. |
| `lmctf19` | Current immutable generation proves 2,153 seeds/34,747 links, then all prune: 631 red-only, 659 blue-only, 0 shared, and 863 neither. The closest opposing seeds are 112 units apart. Automated inventory is exhausted: both flags are symmetric one-way DROP basins, and wider RUN plus reverse JUMP/HOOK/rocket-jump replays prove no egress. It awaits an end-of-plan human traversal. |
| `lmctf25` | The old timeout is gone. Fresh current source proves 2,122 seeds, then all prune: 547 red-only, 557 blue-only, 0 shared, and 1,018 neither. The exact nearest cut is 834.602 units, including 832 units vertically. Two synchronized START_ON trains traverse a sealed fourteen-corner loop and pause for three seconds at authored upper and lower stations 1,408 units apart. Both upper stations have stable red-side seeds and both lower stations have stable blue-side seeds, but current train admission accepts only triggered two-corner trains and therefore emits no ride. A diagnostic two-way station bridge closes 1,104 seeds from both roots and passes generation, both C readers, Python, lint, SNAG bootstrap, and cold load; it is causal evidence only and was not published as fake RUN links. The production repair in progress is an exact continuous-station RL_TRAIN/RLCM_RIDE plan and passive runtime controller. |
...
| `xmap26` | A fresh current-source immutable run corrects the stale dense-water result: sparse-water publication yields 1,174 seeds, 918 exact SWIM links, 11,842 total links, and zero declared lifts; 202 seeds are red-only, 216 blue-only, 0 shared, and 756 neither before all prune. The current closest objective gap is a 1,472-unit static-world cut. The older six-lift, 1,575-seed, 1,440-unit result came from an unmerged dense-water branch and is not current evidence. A current-source exact inverse-boundary experiment admitted only replay-proved objective-improving DROP, HOOK, or rocket-jump reversals; it found zero repair links and reproduced the same failure signature. All diagnostics were reverted. `xmap26` is reserved for an end-of-plan authoritative human traversal. |
...
effects. The resulting evidence identifies the missing generic transition; it
does not authorize a RUNE link until the corresponding oracle replays and proves
that transition. A connected human run remains required to complete each
reserved map's later route update, but it does not block initial release when
the map satisfies the route-only release rule. Current candidates are
`lmctf01`, `lmctf12`, `lmctf19`, `tomb05`, `xmap13`, `xmap18`, and `xmap26`;
each candidate must still prove normal local attack and defense in a real match.
...
5. Require all 175 artifacts to be newly generated from the same freeze,
   ...
   Each map must then have either a route-complete PASS or documented real-match
   proof for the route-only release rule. Any other generation failure,
