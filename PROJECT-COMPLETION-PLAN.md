...
| `tw2ctf4` | Current immutable generation proves 2,066 seeds/31,850 links, then all prune: 596 red-only, 606 blue-only, 0 shared, and 864 neither. The generic authenticated shoot-door pass now resolves all six teams. After ranking candidates by full player-hull distance and restoring each temporary door pose before binding, real generation proves nine crossings and serializes the twelve links needed for objective closure. The resulting core has 1,990 seeds reachable from both objectives. Artifact writing then correctly rejected the late links because their `RL_TRAIN` records lacked declared provenance. The active correction stamps that existing contract and rolls back its counters transactionally before the next immutable smoke. |
