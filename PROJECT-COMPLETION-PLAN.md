...
| Area | Current state |
|---|---|
| Architectural state | Integrated on `slipgate`: frozen contracts; complete BSP, collision, hull configuration, semantic, visibility, hook-surface, and completeness foundations; accepted ground, water, mechanism, and weapon foundations; artifact codec, loader, atomic publication, three independent readers, semantic acceptance, and portable runtime-map preflight; sparse beliefs with exact life identity; weapon profiles; passive human traces; hook-visibility publication; and complete removal of the obsolete SNAG pipeline. Fail-closed ground and hook/air consumer seams are integrated but await accepted phase, host-law, and visibility providers. Shared costs, complete-model proof, semantic publication, and production navigation, strategy, perception, combat, and learning hookups remain open. |
