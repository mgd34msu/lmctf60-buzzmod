...
| Area | Current state |
|---|---|
| Architectural state | The compact static model, analytic contract/evaluator, exact cell/stance localizer, identity-bound wire codec, spatial index and independent readers are focused-test green. Production `sv rune` still emits the legacy seed/link/action graph; the BSP-derived builder, runtime cutover, and legacy deletion remain the critical path. |
