...
| BEL-5 | Store beliefs sparsely as weighted cells/particles or mixtures, including position, velocity, acceleration, orientation, movement/weapon state, confidence, and future time. Team communication may reduce uncertainty only through an authenticated runtime observation. | Sparse identity-bound future prediction is accepted. **Migrate perception and consumers.** | Sight, armor pickup, weapon fire, footsteps, hook, door, lift, water, damage, and team reports produce source-appropriate distributions without dense-map cost. |
