...
Returns ALL entities (even non-solid) that have origins within a spherical area

findallradius (origin, radius)
=================
*/
edict_t *findallradius (edict_t *from, vec3_t org, float rad)
{
	vec3_t	eorg;
	int		j;
...
/*	
	if (!strcmp(ed->classname, "flag") ||
		!strcmp(ed->classname, "damage_rune") ||
		!strcmp(ed->classname, "haste_rune") ||
		!strcmp(ed->classname, "resist_rune") ||
		!strcmp(ed->classname, "regen_rune"))
		assert(false);
*/
	// LM_JORM END
...
void Team_centerprint (int color, char *message, char *elsemessage)
{
    ...
    {
		cl_ent = g_edicts + 1 + i;    //Select the client entity from the list of ents.
        if (!cl_ent->inuse)        //Not in game yet is what I think this means.
			continue;
...
void Team_cprint (int color, char *message, char *elsemessage)
{
    ...
    {
		cl_ent = g_edicts + 1 + i;    //Select the client entity from the list of ents.
        if (!ctf_validateplayer(cl_ent, CTF_TEAM_IGNORETEAM))  //must be a valid player
			continue;
        if (cl_ent->client->ctf.teamnum == color) 
