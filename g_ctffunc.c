...
//	int		i;
	...
	/* TEMPORARY DIAGNOSTIC (bot_developer 1) -- see BotFlagDiag() at the
	 * bottom of this file. Records every entry into the flag pickup. */
	if (gi.cvar("bot_developer", "0", 0)->value)
		gi.dprintf("FLAGDIAG: ctf_flagtouch flagteam=%d toucher=%s bot=%d toucherteam=%d matchstate=%d\n",
			ent->flagteam,
			(other && other->client) ? other->client->pers.netname : "?",
			(other && (other->flags & FL_BOT)) ? 1 : 0,
			(other && other->client) ? other->client->ctf.teamnum : -1,
			matchstate);
 ...
 * Reports where the flag entities really are and how close the bots
 * actually get to them. Remove together with the ctf_flagtouch trace
 * below and the BotFlagDiag() call in G_RunFrame.
 * ------------------------------------------------------------------ */
static float botflagdiag_min[256];
static float botflagdiag_nextprint = 0;
static int   botflagdiag_reported = 0;

/* TEMPORARY DIAGNOSTIC (bot_developer 1) -- what a bot is actually carrying,
 * and where it was standing when it died. Both are here to answer whether a
 * bot on a flag run is equipped to survive the crossing. Remove with the rest
 * of BotFlagDiag. */
static float botflagdiag_kitprint = 0;
static int   botflagdiag_wasalive[256];
static int   botflagdiag_deaths[256];
static int   botflagdiag_deaths_near[256];   /* died within 700 of enemy flag */

/* Weapon and ammo pickup names, in the order they are reported. */
static char *botflagdiag_weapnames[] = {
	"Shotgun", "Super Shotgun", "Machinegun", "Chaingun",
	"Grenade Launcher", "Rocket Launcher", "HyperBlaster",
	"Railgun", "BFG10K"
};
static char botflagdiag_weapletter[] = "sSmcgrhRB";
static char *botflagdiag_ammonames[] = {
...
void BotFlagDiag(void)
{
	...
	if (!botflagdiag_reported && redflag && blueflag)
	{
		botflagdiag_reported = 1;
		gi.dprintf("FLAGDIAG: redflag  cls=%s org=(%.1f %.1f %.1f) mins=(%.0f %.0f %.0f) maxs=(%.0f %.0f %.0f) solid=%d item=%s team=%d touch=%p\n",
			redflag->classname, redflag->s.origin[0], redflag->s.origin[1], redflag->s.origin[2],
			redflag->mins[0], redflag->mins[1], redflag->mins[2],
			redflag->maxs[0], redflag->maxs[1], redflag->maxs[2],
			redflag->solid, redflag->item ? redflag->item->classname : "NULL",
			redflag->flagteam, (void *)redflag->touch);
		gi.dprintf("FLAGDIAG: blueflag cls=%s org=(%.1f %.1f %.1f) mins=(%.0f %.0f %.0f) maxs=(%.0f %.0f %.0f) solid=%d item=%s team=%d touch=%p\n",
			blueflag->classname, blueflag->s.origin[0], blueflag->s.origin[1], blueflag->s.origin[2],
			blueflag->mins[0], blueflag->mins[1], blueflag->mins[2],
			blueflag->maxs[0], blueflag->maxs[1], blueflag->maxs[2],
			blueflag->solid, blueflag->item ? blueflag->item->classname : "NULL",
			blueflag->flagteam, (void *)blueflag->touch);
		for (i = 0; i < 256; i++)
			botflagdiag_min[i] = 99999.0f;
	}

	if (!redflag || !blueflag)
		return;

	for (i = 0; i < game.maxclients && i < 256; i++)
	{
		edict_t *flag;
		cl = g_edicts + 1 + i;
		if (!cl->inuse || !cl->client || !(cl->flags & FL_BOT))
			continue;

		/* distance to the ENEMY flag for this bot */
		flag = (cl->client->ctf.teamnum == CTF_TEAM_RED) ? blueflag : redflag;
		VectorSubtract(flag->s.origin, cl->s.origin, d);
		dist = VectorLength(d);

		/* Alive -> dead transition: say where it happened, what the bot was
		 * holding, and what killed it, so "died on the way in" can be told
		 * apart from "never set off". */
		if (cl->health < 1)
		{
			if (botflagdiag_wasalive[i])
			{
				char kit[128];
				int k, n = 0, w;
				botflagdiag_wasalive[i] = 0;
				botflagdiag_deaths[i]++;
				if (dist < 700.0f)
					botflagdiag_deaths_near[i]++;
				kit[0] = 0;
				for (k = 0; k < (int)(sizeof(botflagdiag_weapnames) /
				                      sizeof(botflagdiag_weapnames[0])); k++)
				{
					gitem_t *it = FindItem(botflagdiag_weapnames[k]);
					if (it && cl->client->pers.inventory[ITEM_INDEX(it)] > 0 &&
					    n < (int)sizeof(kit) - 2)
						kit[n++] = botflagdiag_weapletter[k];
				}
				kit[n] = 0;
				w = cl->client->pers.weapon
				  ? ITEM_INDEX(cl->client->pers.weapon) : -1;
				gi.dprintf("BOTDEATH: %s t%d distflag=%.0f org=(%.0f %.0f %.0f) "
					"weapons=[%s] inhand=%s mod=%d deaths=%d nearflag=%d\n",
					cl->client->pers.netname, cl->client->ctf.teamnum, dist,
					cl->s.origin[0], cl->s.origin[1], cl->s.origin[2],
					kit[0] ? kit : "blaster-only",
					(w >= 0 && cl->client->pers.weapon->pickup_name)
						? cl->client->pers.weapon->pickup_name : "?",
					meansOfDeath, botflagdiag_deaths[i],
					botflagdiag_deaths_near[i]);
			}
			continue;
		}
		botflagdiag_wasalive[i] = 1;

		if (dist < botflagdiag_min[i])
			botflagdiag_min[i] = dist;
	}

	/* Every 20s: what each bot is actually carrying. A bot that never
	 * upgrades past the blaster cannot fight its way across the map. */
	if (level.time >= botflagdiag_kitprint)
	{
		char buf[1024];
		int n = 0;
		botflagdiag_kitprint = level.time + 20.0f;
		buf[0] = 0;
		for (i = 0; i < game.maxclients && i < 256; i++)
		{
			int k, nw = 0, ammo = 0;
			                      ...
			                      sizeof(botflagdiag_weapnames[0])); k++)
			{
				...
				if (it && cl->client->pers.inventory[ITEM_INDEX(it)] > 0 &&
				    nw < (int)sizeof(kit) - 1)
					kit[nw++] = botflagdiag_weapletter[k];
			}
			kit[nw] = 0;
			for (k = 0; k < (int)(sizeof(botflagdiag_ammonames) /
			                      sizeof(botflagdiag_ammonames[0])); k++)
			{
				gitem_t *it = FindItem(botflagdiag_ammonames[k]);
				if (it) ammo += cl->client->pers.inventory[ITEM_INDEX(it)];
			}
			n += sprintf(buf + n, "%s:%s/a%d/h%d/d%d ",
				cl->client->pers.netname, kit[0] ? kit : "-",
				ammo, cl->health, botflagdiag_deaths[i]);
	...
	if (level.time >= botflagdiag_nextprint)
	{
		char buf[1024];
		int n = 0;
		botflagdiag_nextprint = level.time + 10.0f;
		buf[0] = 0;
		for (i = 0; i < game.maxclients && i < 256; i++)
		{
			cl = g_edicts + 1 + i;
			if (!cl->inuse || !cl->client || !(cl->flags & FL_BOT))
				continue;
			n += sprintf(buf + n, "%d/%s/t%d:%.0f ", i,
				cl->client->pers.netname, cl->client->ctf.teamnum,
				botflagdiag_min[i]);
			if (n > (int)sizeof(buf) - 24)
				break;
		}
		gi.dprintf("FLAGDIAG: closest approach to enemy flag so far: %s\n", buf);
	}
}
