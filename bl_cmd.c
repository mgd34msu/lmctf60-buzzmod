...
#include "bl_botcfg.h"
#include "g_tourney.h"	/* SetPause, GamePaused */
#include "bl_ctf.h"
...
qboolean BotServerCmd(char *cmd, edict_t *ent, int server)
{
	...
	else if (Q_stricmp(cmd, "botpause") == 0)
	{
		...
		else
		{
			...
			{
				/* LMCTF has its own referee pause (match_pause, driven by
				 * SetPause), so use it rather than a second flag of our own.
				 * SetPause does the client bookkeeping and announcing. */
				SetPause(!GamePaused());
				gi.bprintf(PRINT_HIGH, "%s %s the game\n",
					ent ? ent->client->pers.netname : "console",
					GamePaused() ? "paused" : "unpaused");
			} //end else
		} //end else
	} //end else if
	else if (ent && Q_stricmp(cmd, "menu") == 0)
	{
		/* Rebuilt on this mod's own menu system -- see g_botmenu.c. */
		Ref_Bot_Menu(ent);
	} //end else if
	else
	{
