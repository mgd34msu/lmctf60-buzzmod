...
/* Link_Doors runs before objective-aware door planning, when the ordinary, swim,
 * lift,
...
/* Select the nearest structurally linked seed at an objective stand. */
static int Graph_ObjectiveRoot(const vec3_t objective, const byte *has_out,
	graph_objective_diag_t *diag)
{
...
static qboolean Rune_GenerateMode(const char *mapname)
{
	...
	sg_host.dprint("rune: door-drop compound trials=%d proofs=%d\n",
	               gen_door_drop_compound_trials,
	               gen_door_drop_compound_proofs);
	/* Structural topology is authoritative.  Objective reachability remains a
	 * runtime destination diagnostic and never prunes or validates the artifact. */
...
qboolean Rune_Generate(const char *mapname) { return Rune_GenerateMode(mapname); }
qboolean Rune_Update(const char *mapname) { return Rune_GenerateMode(mapname); }
