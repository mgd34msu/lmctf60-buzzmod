...
static int EvaluateFunction(const sg_rune_analytic_table_t *table,
	uint32_t function, const float inputs[SG_RUNE_ANALYTIC_INPUT_COUNT],
	float *value_out, uint32_t depth);

static int EvaluatePolynomial(const sg_rune_analytic_table_t *table,
	const sg_rune_analytic_function_t *record,
	const float inputs[SG_RUNE_ANALYTIC_INPUT_COUNT], float *value_out)
{
	float total = 0.0f;
	uint32_t index;

	if (record->input_count > SG_RUNE_ANALYTIC_MAX_INPUTS ||
		record->first > table->term_count ||
		record->count > table->term_count - record->first)
		return 0;
	for (index = 0U; index < record->input_count; index++)
		if (record->inputs[index] >= SG_RUNE_ANALYTIC_INPUT_COUNT)
			return 0;
	for (index = 0U; index < record->count; index++)
	{
		const sg_rune_analytic_term_t *term = &table->terms[record->first + index];
...
static int EvaluatePiecewise(const sg_rune_analytic_table_t *table,
	const sg_rune_analytic_function_t *record,
	const float inputs[SG_RUNE_ANALYTIC_INPUT_COUNT], float *value_out,
	uint32_t depth)
{
	...
	for (index = 0U; index < record->count; index++)
	{
		const sg_rune_analytic_clause_t *clause =
			&table->clauses[record->first + index];
		float selector;

		if (clause->input >= SG_RUNE_ANALYTIC_INPUT_COUNT)
			return 0;
	...
	float *value_out, uint32_t depth)
{
	...
	switch (record->kind)
	{
	case SG_RUNE_ANALYTIC_POLYNOMIAL:
		return EvaluatePolynomial(table, record, inputs, value_out);
	case SG_RUNE_ANALYTIC_PIECEWISE:
...
int SG_RuneAnalyticEvaluate(const sg_rune_analytic_table_t *table,
	uint32_t function, const float inputs[SG_RUNE_ANALYTIC_INPUT_COUNT],
	float *value_out)
{
	uint32_t index;
	...
	*value_out = 0.0f;
	if (!table || !inputs)
		return 0;
	for (index = 0U; index < SG_RUNE_ANALYTIC_INPUT_COUNT; index++)
		if (!Finite(inputs[index]))
			return 0;
	return EvaluateFunction(table, function, inputs, value_out, 0U);
}

int SG_RuneAnalyticTableValid(const sg_rune_analytic_table_t *table)
{
	uint32_t index;
	...
	{
		const sg_rune_analytic_function_t *record = &table->functions[index];
		uint32_t slot;

		if (record->output >= SG_RUNE_ANALYTIC_OUTPUT_COUNT ||
			record->reserved != 0U)
			return 0;
		if (record->kind == SG_RUNE_ANALYTIC_POLYNOMIAL)
		{
			if (record->input_count > SG_RUNE_ANALYTIC_MAX_INPUTS ||
				record->first > table->term_count ||
				record->count > table->term_count - record->first)
				return 0;
			for (slot = 0U; slot < record->input_count; slot++)
				if (record->inputs[slot] >= SG_RUNE_ANALYTIC_INPUT_COUNT)
					return 0;
			for (slot = 0U; slot < record->count; slot++)
			{
				const sg_rune_analytic_term_t *term =
				...
				for (axis = record->input_count;
					axis < SG_RUNE_ANALYTIC_MAX_INPUTS; axis++)
					if (term->exponents[axis] != 0U)
						return 0;
			}
		}
		else if (record->kind == SG_RUNE_ANALYTIC_PIECEWISE)
		{
			if (record->first > table->clause_count ||
				record->count > table->clause_count - record->first ||
				record->count == 0U)
			...
			for (slot = 0U; slot < record->count; slot++)
			{
				...
				if (clause->function >= table->function_count ||
					...
					clause->input >= SG_RUNE_ANALYTIC_INPUT_COUNT ||
...
void SG_RuneAnalyticStoreInit(sg_rune_analytic_store_t *store)
{
	if (store)
		memset(store, 0, sizeof(*store));
}

void SG_RuneAnalyticStoreFree(sg_rune_analytic_store_t *store)
{
	if (!store)
		return;
...
void SG_RuneAnalyticStoreView(const sg_rune_analytic_store_t *store,
	sg_rune_analytic_table_t *table_out)
{
	if (!table_out)
		return;
...
static uint32_t AppendFunction(sg_rune_analytic_store_t *store,
	const sg_rune_analytic_function_t *record)
{
	if (!Grow((void **)&store->functions, &store->function_capacity,
		store->function_count + 1U, sizeof(*store->functions)))
		return SG_RUNE_ANALYTIC_INDEX_NONE;
	store->functions[store->function_count] = *record;
	return store->function_count++;
}

uint32_t SG_RuneAnalyticAppendPolynomial(sg_rune_analytic_store_t *store,
	sg_rune_analytic_output_t output, const uint8_t *inputs,
	uint32_t input_count, const sg_rune_analytic_term_t *terms,
	uint32_t term_count)
{
	sg_rune_analytic_function_t record;
	uint32_t index;

	if (!store || input_count > SG_RUNE_ANALYTIC_MAX_INPUTS ||
		(input_count && !inputs) || (term_count && !terms) ||
		output >= SG_RUNE_ANALYTIC_OUTPUT_COUNT)
		return SG_RUNE_ANALYTIC_INDEX_NONE;
	for (index = 0U; index < input_count; index++)
		if (inputs[index] >= SG_RUNE_ANALYTIC_INPUT_COUNT)
			return SG_RUNE_ANALYTIC_INDEX_NONE;
	for (index = 0U; index < term_count; index++)
	{
		...
		if (!Finite(terms[index].coefficient))
			return SG_RUNE_ANALYTIC_INDEX_NONE;
		for (axis = input_count; axis < SG_RUNE_ANALYTIC_MAX_INPUTS; axis++)
	...
	record.kind = SG_RUNE_ANALYTIC_POLYNOMIAL;
...
uint32_t SG_RuneAnalyticAppendConstant(sg_rune_analytic_store_t *store,
	sg_rune_analytic_output_t output, float value)
{
	sg_rune_analytic_term_t term;

	memset(&term, 0, sizeof(term));
	term.coefficient = value;
	return SG_RuneAnalyticAppendPolynomial(store, output, NULL, 0U, &term, 1U);
}

uint32_t SG_RuneAnalyticAppendAffine(sg_rune_analytic_store_t *store,
	sg_rune_analytic_output_t output, const uint8_t *inputs,
	uint32_t input_count, float bias, const float *slopes)
{
	sg_rune_analytic_term_t terms[SG_RUNE_ANALYTIC_MAX_INPUTS + 1U];
	uint32_t index, count = 0U;

	if (input_count > SG_RUNE_ANALYTIC_MAX_INPUTS || (input_count && !slopes))
		return SG_RUNE_ANALYTIC_INDEX_NONE;
	...
	return SG_RuneAnalyticAppendPolynomial(store, output, inputs, input_count,
		terms, count);
}

uint32_t SG_RuneAnalyticAppendPiecewise(sg_rune_analytic_store_t *store,
	sg_rune_analytic_output_t output, const sg_rune_analytic_clause_t *clauses,
	uint32_t clause_count)
{
	...
	if (!store || !clauses || !clause_count ||
		...
		return SG_RUNE_ANALYTIC_INDEX_NONE;
	memset(&record, 0, sizeof(record));
	record.kind = SG_RUNE_ANALYTIC_PIECEWISE;
	record.output = (uint8_t)output;
