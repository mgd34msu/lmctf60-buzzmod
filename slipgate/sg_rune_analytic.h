...
#ifndef SG_RUNE_ANALYTIC_H
#define SG_RUNE_ANALYTIC_H

#include <stdint.h>

#define SG_RUNE_ANALYTIC_MAX_INPUTS 4U
#define SG_RUNE_ANALYTIC_INDEX_NONE UINT32_MAX

typedef enum sg_rune_analytic_input_e
{
	...
	SG_RUNE_ANALYTIC_INPUT_COUNT
} sg_rune_analytic_input_t;

typedef enum sg_rune_analytic_output_e
{
	...
	SG_RUNE_ANALYTIC_OUTPUT_REACHABILITY,   /* > 0 reachable, < 0 not */
	SG_RUNE_ANALYTIC_OUTPUT_COUNT
} sg_rune_analytic_output_t;

typedef enum sg_rune_analytic_kind_e
{
	SG_RUNE_ANALYTIC_POLYNOMIAL = 0,
	SG_RUNE_ANALYTIC_PIECEWISE,
	SG_RUNE_ANALYTIC_KIND_COUNT
} sg_rune_analytic_kind_t;

/* coefficient * product over inputs[i] ^ exponents[i]. */
typedef struct sg_rune_analytic_term_s
{
	float coefficient;
	uint8_t exponents[SG_RUNE_ANALYTIC_MAX_INPUTS];
} sg_rune_analytic_term_t;

/* Selected when lower <= input < upper (inclusive upper for the last clause
 * is the writer's business: give it +infinity). */
typedef struct sg_rune_analytic_clause_s
{
	uint32_t function;
	uint8_t input;
	uint8_t reserved[3];
	float lower;
	float upper;
} sg_rune_analytic_clause_t;

typedef struct sg_rune_analytic_function_s
{
	uint8_t kind;             /* sg_rune_analytic_kind_t */
	uint8_t output;           /* sg_rune_analytic_output_t */
	uint8_t input_count;      /* polynomial: inputs used by the terms */
	uint8_t inputs[SG_RUNE_ANALYTIC_MAX_INPUTS];
	uint8_t reserved;
	uint32_t first;           /* first term, or first clause */
	uint32_t count;
} sg_rune_analytic_function_t;

typedef struct sg_rune_analytic_table_s
{
	const sg_rune_analytic_function_t *functions;
	uint32_t function_count;
	const sg_rune_analytic_term_t *terms;
	uint32_t term_count;
	const sg_rune_analytic_clause_t *clauses;
	uint32_t clause_count;
} sg_rune_analytic_table_t;
 ...
 * Returns the function index or SG_RUNE_ANALYTIC_INDEX_NONE. */
