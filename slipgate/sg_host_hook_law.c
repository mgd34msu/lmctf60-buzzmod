...
int SG_HostHookLawValid(const sg_host_hook_law_t *law)
{
	...
	return law->version == SG_HOST_HOOK_LAW_VERSION &&
		law->trace_mask == MASK_SHOT &&
		law->muzzle_forward_offset == SG_HOST_HOOK_MUZZLE_FORWARD_OFFSET &&
		law->muzzle_right_offset == SG_HOST_HOOK_MUZZLE_RIGHT_OFFSET &&
		law->muzzle_view_offset == SG_HOST_HOOK_MUZZLE_VIEW_OFFSET &&
		...
		memcmp(&law->trace_epsilon,
			&(const float){ SG_HOST_HOOK_TRACE_EPSILON },
			sizeof(law->trace_epsilon)) == 0 &&
	...
	const float forward[3], const float right[3], float start_out[3])
{
	...
	if (!FiniteVector(origin) || !FiniteVector(forward) ||
		...
		return 0;
	forward_offset = (float)SG_HOST_HOOK_MUZZLE_FORWARD_OFFSET;
	side = hand == SG_HOST_HOOK_LEFT_HAND ?
		-(float)SG_HOST_HOOK_MUZZLE_RIGHT_OFFSET :
		hand == SG_HOST_HOOK_CENTER_HAND ? 0.0f :
		(float)SG_HOST_HOOK_MUZZLE_RIGHT_OFFSET;
	view_offset = viewheight - (float)SG_HOST_HOOK_MUZZLE_VIEW_OFFSET;
...
static void ClearTarget(sg_host_hook_step_t *step_out)
{
	step_out->target_kind = SG_HOST_HOOK_TARGET_NONE;
	step_out->target_identity = 0U;
}

static void AbortHook(sg_host_hook_step_t *step_out)
{
	step_out->aborted = 1;
	step_out->next_phase = SG_HOST_HOOK_IDLE;
	ClearTarget(step_out);
}

static void ApplyFlightHit(const sg_host_hook_law_t *law,
	const sg_host_hook_observation_t *input, sg_host_hook_step_t *step_out)
{
	if (input->owner_hit)
		return;
	...
	sg_host_hook_step_t *step_out)
{
		...
		input->event == SG_HOST_HOOK_REFIRE)
	{
		...
		if ((input->event == SG_HOST_HOOK_FIRE && input->phase != SG_HOST_HOOK_IDLE) ||
			(input->event == SG_HOST_HOOK_REFIRE && input->phase != SG_HOST_HOOK_COAST &&
				input->phase != SG_HOST_HOOK_IDLE) || !input->attack_held)
			return 1;
		step_out->accepted = 1;
		step_out->next_phase = SG_HOST_HOOK_IN_FLIGHT;
		/* Spawn/state advance precedes the owner-to-bolt trace.  A blocked
		 * muzzle therefore still spawns the bolt and immediately dispatches
	...
	if (input->event == SG_HOST_HOOK_RELEASE)
	{
		...
		if (input->grounded)
		{
			/* ctf_hook_abort clears both vertical components only when the
			 * player has a groundentity.  Horizontal coast remains irrelevant
			 * to the hook state machine. */
			step_out->zero_velocity_z = 1;
			step_out->zero_oldvelocity_z = 1;
			step_out->coast_velocity = 0;
		}
		else
			step_out->coast_velocity = 1;
		return 1;
