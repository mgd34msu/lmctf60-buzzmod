...
int SG_RuneVisInit(sg_rune_vis_t *vis, const sg_rune_bsp_t *bsp)
{
	if (!vis || !bsp)
		return 0;
...
int32_t SG_RuneVisClusterAt(const sg_rune_bsp_t *bsp, const float point[3])
{
	int32_t node;
	...
	while (node >= 0)
	{
		...
		float side = plane->normal[0] * point[0] +
...
{
	const sg_rune_bsp_visibility_t *data;
	...
	if (cluster < 0 || (uint32_t)cluster >= data->cluster_count || !data->bytes ||
		!data->pvs_offsets)
		return;
	offset = data->pvs_offsets[cluster];
