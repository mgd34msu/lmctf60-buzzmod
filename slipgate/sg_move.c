...
static qboolean MechanismStep_StockPlatformDoorPlan(const sg_bot_t *bot,
	sg_rune_mechanism_binding_t *binding_out)
{
	uint32_t keys[SG_RUNE_BINDING_MAX_MOVERS];
	size_t count;
	...
	return MechanismStep_Binding(bot, RL_LIFT, binding_out) &&
	       ...
	       SG_RuneMechanismBindingPlatformAutoDoorStage(binding_out,
	           &trigger, keys, &count);
}
...
static qboolean MechanismStep_PlatformRideStateBegin(sg_bot_t *bot,
	const sg_rune_mechanism_binding_t *binding)
{
	if (!binding || !binding->link || binding->link->mode != RLCM_RIDE ||
	    !MechanismStep_CarrierStateBegin(bot, binding))
		return false;
...
qboolean SG_AuthorizeDoorTriggerTouch(edict_t *source, edict_t *activator)
{
	...
	{
		int compound = SG_CompoundHookGameAuthorizeTouch(bot, source,
		    activator, level.framenum);
	...
	edict_t *activator)
{
	...
	if (binding.link->mode == RLCM_RIDE &&
	    (bot->carrier_action.phase != SG_CARRIER_PHASE_CARRIER_READY ||
	     ...
	     !SG_LiftRider(platform, activator)))
		return false;
	if (binding.link->mode == RLCM_NONE &&
	    binding.plan->expected_members > 1U)
	{
		if (bot->carrier_action.phase != SG_CARRIER_PHASE_CARRIER_READY ||
		    !MechanismStep_CarrierStateCurrent(bot, &binding) ||
	...
	edict_t *activator)
{
	...
	{
		int compound = SG_CompoundHookGameAuthorizeActivation(bot, source,
		    door_master, activator, level.framenum);
...
static sg_bot_t *TraversalLiveEventOwner(edict_t *activator)
{
	int i;
	...
	for (i = 0; i < SG_MAXBOTS; i++)
		if (sg_bots[i].active && sg_bots[i].ent == activator)
		{
		    ...
		    sg_bots[i].compound_hook_live.guard_owned)
				return &sg_bots[i];
		}
	return NULL;
}
...
{
	sg_bot_t *bot = TraversalLiveEventOwner(activator);
	...
	if (bot->compound_hook_live.guard_owned)
		SG_CompoundHookGameObserveSafety(bot, door_passed, contaminated);
}
...
void SG_NoteDropSolidContact(edict_t *source, edict_t *activator)
{
	    ...
	    strncmp(source->classname, "func_door", 9) == 0)
	{
		if (bot->drop_started && bot->drop_replay_active)
			(void)SG_DropLiveEventsLatch(
			    &bot->drop_live_events, false, true);
		if (bot->compound_drop_live.guard_owned &&
		    source->s.number != bot->compound_drop_live.snapshot.mover_key)
			(void)SG_DropLiveEventsLatch(
			    &bot->compound_drop_live.drop_events, false, true);
		if (bot->compound_hook_live.guard_owned &&
		    source->s.number != bot->compound_hook_live.snapshot.mover_key)
			SG_CompoundHookGameObserveSafety(bot, true, false);
	}
	else
	{
		if (bot->drop_started && bot->drop_replay_active)
...
void Think_Emit(sg_bot_t *bot, sg_think_t *tc)
{
	...
	qboolean compound_hook = (bestlink >= 0 && SG_Rune() &&
	    SG_Rune()->links && bestlink < SG_Rune()->hdr.num_links &&
	    SG_Rune()->links[bestlink].action == RL_DOOR_HOOK);
	...
	qboolean proved_control = proved_ballistic || proved_swim ||
	    compound_drop || compound_hook || declared_control;
	...
	for (step = 0; step < sub; step++)
		{
			...
			if (declared_control)
			{
				...
				qboolean compound_carrier = decl->action == RL_LIFT &&
				    ...
				    mechanism_binding.entry_node->touch_callback ==
				        SG_MECH_CALLBACK_TOUCH_MULTI;
				qboolean platform_ride = decl->action == RL_LIFT &&
				    decl->mode == RLCM_RIDE && mechanism_binding.plan &&
				    mechanism_binding.plan->controller_kind ==
				        SG_MECHANISM_CONTROLLER_PLATFORM &&
				    mechanism_binding.entry_node->touch_callback ==
				        SG_MECH_CALLBACK_TOUCH_PLAT_CENTER;
				qboolean platform_door_ascent = decl->action == RL_LIFT &&
				    decl->mode == RLCM_NONE && mechanism_binding.plan &&
				    mechanism_binding.plan->controller_kind ==
				        SG_MECHANISM_CONTROLLER_PLATFORM &&
				    mechanism_binding.plan->expected_members > 1U &&
				    mechanism_binding.entry_node->touch_callback ==
						...
						if ((platform_ride &&
						    ...
						    (platform_door_ascent &&
						     MechanismStep_CarrierStateBegin(bot,
						         &mechanism_binding) &&
						     SG_DeclaredCarrierDoorGuardAcquire(bot, bestlink,
						         SG_CARRIER_DOOR_APPROACH) ==
						         SG_COMPOUND_GUARD_OK) ||
						    (!platform_ride && !platform_door_ascent &&
						     !compound_carrier) ||
						    (!platform_ride && !platform_door_ascent &&
						     compound_carrier &&
						     MechanismStep_CarrierStateBegin(bot,
				    ...
				    !bot->declared_activated)
				{
					...
					else if (platform_ride && !bot->declared_touched &&
					             ...
					             SG_CARRIER_PHASE_CARRIER_READY)
					{
						...
						{
							if (SG_DeclaredCarrierDoorGuardAcquire(bot,
							        bestlink, SG_CARRIER_DOOR_EGRESS) !=
							    SG_COMPOUND_GUARD_OK)
							{
								bot->carrier_action.phase =
					...
					else if (platform_door_ascent)
					{
						sg_carrier_action_state_t *carrier =
							&bot->carrier_action;

						if (bot->declared_touched && !SG_LiftRider(plat, e))
						{
							carrier->phase = SG_CARRIER_PHASE_FAILED;
							bot->commit_link = -1;
							hold = true;
						}
						if (carrier->phase ==
						...
						if (carrier->phase ==
							...
							hold = true;
						         ...
						         SG_CARRIER_PHASE_CARRIER_READY)
						{
							if (SG_DeclaredDoorGuardHoldOpen(bot, 1000) !=
							        SG_COMPOUND_GUARD_OK ||
							    !SG_LiftRest(mechanism_binding.entry_entity,
							        plat, e, carrier_body))
							{
					...
					if (platform_ride && plat && bot->commit_link >= 0 &&
					    bot->declared_touched &&
					    plat->moveinfo.state == SG_PLAT_STATE_BOTTOM &&
					    bot->carrier_action.phase >=
					        SG_CARRIER_PHASE_EGRESS_ARMED)
				...
				if (platform_ride && bot->declared_activated &&
				    bot->carrier_action.phase == SG_CARRIER_PHASE_EGRESS_OPEN &&
				    SG_SupportedArrived(e->s.origin,
				        SG_Rune()->seeds[decl->to].origin,
				        e->groundentity != NULL, e->watertype, e->waterlevel, e) &&
				    !SG_CarrierActionComplete(bot, &mechanism_binding))
					hold = true;
				...
				SG_DropLiveZeroCommand(cmd);
			{
				...
				if (compound_hook && bot->compound_hook_live.guard_owned)
				{
					...
					if (!SG_CompoundHookGameHost(bot, &host) ||
					    !SG_CompoundHookGamePose(e, &pose) ||
					    !SG_CompoundHookGameObservation(bot, e, &observation))
					{
						SG_DeclaredDoorTerminalDeath(bot);
						return;
					}
					result = SG_CompoundHookLivePreStep(
					    &bot->compound_hook_live, &host, &pose,
					    &observation, cmd);
					if (!result.command_ready &&
					    result.outcome == SG_COMPOUND_HOOK_LIVE_RECOVERING)
						result = SG_CompoundHookGameRecoverOwnedFailure(
						    bot, cmd);
					if (!result.command_ready)
					{
						if (result.outcome ==
					...
					result = SG_CompoundHookLiveApproveCommand(
					    &bot->compound_hook_live, cmd);
					if (!bot->compound_hook_live.command_pending ||
			...
			if (compound_hook && bot->compound_hook_live.guard_owned)
			{
				...
				if (!SG_CompoundHookGameHost(bot, &host) ||
				    !SG_CompoundHookGamePose(e, &pose) ||
				    !SG_CompoundHookGameTakeObservation(bot, e, &observation))
				{
					SG_DeclaredDoorTerminalDeath(bot);
					return;
				}
				result = step < sub - 1 ?
				    SG_CompoundHookLivePostStep(&bot->compound_hook_live, &host,
				        &pose, &observation) :
				    SG_CompoundHookLiveBoundary(&bot->compound_hook_live, &host,
				        &pose, &observation);
				if (result.outcome == SG_COMPOUND_HOOK_LIVE_RECOVERING)
					result = SG_CompoundHookGameRecoverOwnedFailure(bot, NULL);
				...
				if (bot->compound_hook_live.hook.release_requested &&
				    ...
				    !SG_CompoundHookGameApplyRequestedRelease(bot))
				{
					SG_DeclaredDoorTerminalDeath(bot);
					return;
				}
				if (step == sub - 1 &&
				    ...
				    !bot->compound_hook_live.bolt_linked)
				{
