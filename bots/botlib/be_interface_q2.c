...
/*
 * Travel flags for routing and movement.
 *
 * TFL_DEFAULT deliberately leaves out TFL_GRAPPLEHOOK -- in Quake III the game
 * DLL ORs it in per bot each frame:
 *
 *     bs->tfl = TFL_DEFAULT;
 *     if (bot_grapple.integer) bs->tfl |= TFL_GRAPPLEHOOK;
 *                                              (game_q3/ai_dmnet.c:1469)
 *
 * This adapter passed the bare constant, so every TRAVEL_GRAPPLEHOOK
 * reachability the AAS compiler produced was routed around and the grapple
 * movement code (BotTravel_Grapple) never ran. LMCTF's hook is offhand and
...
static int Q2BotTravelFlags(void)
{
    ...
    if (LibVarGetValue("bot_grapple"))
        tfl |= TFL_GRAPPLEHOOK;
    if (LibVarGetValue("bot_rocketjump"))
        tfl |= TFL_ROCKETJUMP;
...
ctf_navigate:
    ...
    if (bc->hasgoal) {
        ...
        if (BotGetTopGoal(bc->goalstate, &active_goal)) {
            ...
            BotMoveToGoal(moveresult, bc->movestate, &active_goal, Q2BotTravelFlags());
...
static int Q2AAS_AreaCenter(int areanum, vec3_t center)
{
    extern aas_t aasworld;
...
#endif
{
    ...
    /* On by default: LMCTF's hook is offhand, so a bot always has it. The
     * game DLL still has to publish the hook's model index above before
