...
class SnagRepairToolTest(unittest.TestCase):
    ...
    def test_bootstrap_corpus_is_complete_explicit_and_non_overwriting(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(document["map_count"], 181)
            self.assertEqual(len(document["maps"]), 181)
            self.assertEqual(len(list((output / "maps").glob("*.snag"))), 181)
            self.assertEqual(
                len(list((output / "evidence").glob("*.json"))), 181)
