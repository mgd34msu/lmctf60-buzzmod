...
for cc in gcc clang
do
	$cc $strict $test_flags $isl_cflags -I. $sources -lm \
		-o "$tmp_dir/rune-compact-builder-$cc"
	"$tmp_dir/rune-compact-builder-$cc"
	$cc $strict -O3 -ffp-contract=fast $test_flags $isl_cflags -I. \
...
clang $strict $test_flags $isl_cflags $real_flags -fno-omit-frame-pointer \
