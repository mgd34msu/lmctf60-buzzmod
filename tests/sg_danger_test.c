/* Focused model and payload-codec tests for graph-bound danger. */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
typedef struct danger_fixture_s
{
	...
	unsigned char mechanism_strings[1];
} danger_fixture_t;
...
static void FixtureInit(danger_fixture_t *fixture)
{
	...
	fixture->rune.artifact.magic = RUNE_ARTIFACT_MAGIC;
	fixture->rune.artifact.payload_crc32 = UINT32_C(0x10203040);
	fixture->rune.artifact.header_crc32 = UINT32_C(0x50607080);
	fixture->rune.artifact.action_contract_crc32 =
		SG_RUNE_ACTION_CONTRACT_CRC32;
	fixture->rune.artifact.mechanism_contract_crc32 =
		UINT32_C(0x90a0b0c0);
	fixture->rune.artifact.num_seeds = TEST_SEEDS;
	fixture->rune.artifact.num_links = 3U;
	fixture->rune.artifact.string_bytes = 1U;
	fixture->rune.artifact.identity.bsp_checksum = UINT32_C(0x11223344);
	fixture->rune.artifact.identity.entity_crc32 = UINT32_C(0x55667788);
	fixture->rune.artifact.identity.physics_flags =
		SG_RUNE_PROOF_PHYSICS_FLAGS_SUPPORTED;
	fixture->rune.artifact.identity.gravity = 800.0f;
	fixture->rune.artifact.identity.airaccelerate = 0.0f;
	fixture->rune.artifact.identity.maxvelocity = 2000.0f;
	fixture->rune.artifact.identity.pmove_substep_ms =
		SG_RUNE_PROOF_PMOVE_SUBSTEP_MS;
	fixture->rune.artifact.identity.server_frame_ms =
		SG_RUNE_PROOF_SERVER_FRAME_MS;
	fixture->rune.artifact.identity.host_physics_id = 1U;
	memcpy(fixture->rune.artifact.identity.map_name, "danger_test",
		sizeof("danger_test"));

	fixture->rune.hdr.magic = (int)RUNE_ARTIFACT_MAGIC;
	fixture->rune.hdr.num_seeds = (int)TEST_SEEDS;
	fixture->rune.hdr.num_links = 3;
	memcpy(fixture->rune.hdr.mapname,
	    fixture->rune.artifact.identity.map_name,
	...
	fixture->rune.mechanism_strings = fixture->mechanism_strings;
	fixture->linked[0] = 1;
	fixture->linked[1] = 1;
	fixture->linked[2] = 0;
	fixture->linked[3] = 1;
...
static void TestResetAndCandidateCodec(void)
{
	...
	CHECK(Danger_PayloadBytes(&fixture.rune) == TEST_PAYLOAD_BYTES);
	...
	CHECK(Danger_DecodeCandidate(&fixture.rune, payload,
	...
	memcpy(saved, payload, sizeof(saved));
	CHECK(!Danger_CapturePayload(payload, sizeof(payload),
...
static void TestPublicationBindingAndCapture(void)
{
	...
	CHECK(Danger_CapturePayload(captured, sizeof(captured),
	...
	CHECK(!Danger_CapturePayload(captured, sizeof(captured) - 1,
	    &captured_size, &captured_revision));
	...
	CHECK(!Danger_CapturePayload(payload_alias.bytes,
	    TEST_PAYLOAD_BYTES,
	...
	before = Danger_Revision();
	fixture.rune.artifact.identity.gravity = 801.0f;
	CHECK(!Danger_CapturePayload(captured, sizeof(captured),
	    &captured_size, &captured_revision));
	Danger_Learn(1, 0);
	CHECK(Danger_Revision() == before);
...
int main(void)
{
	...
	if (failures)
	{
		fprintf(stderr, "sg_danger_test: %d failure(s)\n", failures);
		return 1;
	}
	puts("sg_danger_test: ok");
	return 0;
}
