...
HOOK_GAME = (ROOT / "slipgate" / "sg_hook_game.c").read_text()
...
assert fire.index("Hook_DiagnosticBegin(bot, role);") < fire.index("SG_HookGameBeginAfterFire")
assert 'SG_HookGameFailDetail(e, bot, 15.0f, "begin-failed")' in fire
assert "Hook_DiagnosticAnchorQ8" in MOVE
assert "bot->instance_token,\n\t    Hook_DiagnosticEmit, NULL" in MOVE

arrived = HOOK_GAME[HOOK_GAME.index("static qboolean Hook_LiveRetireNonRunning"):HOOK_GAME.index("static qboolean Hook_LiveWaitAttachFrame")]
assert 'SG_HookDiagnosticsFinish(&bot->hook_diagnostics,\n\t\t    "arrived", "reducer")' in arrived
graph_fail = HOOK_GAME[HOOK_GAME.index("void SG_HookGameFailDetail"):HOOK_GAME.index("void SG_HookGameFail(")]
assert graph_fail.index("SG_HookDiagnosticsFinish") < graph_fail.index("ctf_hook_abort")
assert "SG_ReplayReasonName(result->replay_reason)" in HOOK_GAME
...
release = HOOK_GAME[
    HOOK_GAME.index("SG_HookLiveReleaseApplied"):
    HOOK_GAME.index("if (!failed && release_seen)")
]
