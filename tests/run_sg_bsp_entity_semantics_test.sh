...
sources='tests/sg_bsp_entity_semantics_test.c slipgate/sg_bsp_entity_semantics.c'

cd "$repo_dir"
for cc in gcc clang
do
	$cc $strict -I. $sources -lm -o "$tmp_dir/entity-$cc"
	"$tmp_dir/entity-$cc"
done

clang $strict -fno-omit-frame-pointer -fsanitize=address,undefined \
	-I. $sources -lm -o "$tmp_dir/entity-sanitize"
ASAN_OPTIONS=detect_leaks=1:halt_on_error=1 \
	UBSAN_OPTIONS=halt_on_error=1:print_stacktrace=1 \
	"$tmp_dir/entity-sanitize"
