...
def test_carrier_screen_uses_the_accepted_moving_formation_by_default():
    ...
    assert "SG_FieldCarrierTrailStation(SG_Rune(), cf, oc->seed" in interpose
    assert "SG_FieldNearestBandSeed(SG_Rune(), cf, oc->seed" not in interpose
    assert "SG_InterposeFallbackSeed(interpose_mode," in interpose
