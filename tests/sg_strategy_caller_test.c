...
static int failures;
static int preparation_field[2] = { 900, 0 };
...
typedef struct caller_fixture_s
{
	...
	sg_destination_field_t fields[3];
...
static void InitFixture(caller_fixture_t *fixture)
{
	...
	fixture->fields[0] = (sg_destination_field_t){
		...
		.generation = 1U,
		.computed_at_ms = 100U,
		.destination = Handle(SG_DESTINATION_POWERUP, 101U),
		.samples = fixture->samples,
		.sample_count = 1U,
		.complete = 1U
	};
	fixture->fields[1] = fixture->fields[0];
	fixture->fields[1].destination = Handle(SG_DESTINATION_FLAG, 201U);
	fixture->fields[2] = fixture->fields[0];
	fixture->fields[2].destination = Handle(SG_DESTINATION_FLAG, 202U);
}
...
static void AddBinding(sg_strategy_caller_plan_t *plan, uint16_t index,
	...
	int role, const int *execution_field, const caller_fixture_t *fixture,
	...
	sg_strategy_principal_kind_t principal, uint32_t principal_id)
{
	...
	sg_strategy_goal_spec_t *preparation;
	...
	preparation = &plan.spec.goals[0];
	InitGoal(preparation, 1U, SG_STRATEGY_GOAL_COLLECT_ITEM, INT16_C(20));
	preparation->failure.try_alternatives = 0U;
	preparation->choice_count = 1U;
	preparation->choices[0] = (sg_strategy_target_choice_t){
		.id = 1U,
		.destination = {
			.kind = SG_DESTINATION_POWERUP,
			.value.item = { 101U }
		}
	};
	primary = &plan.spec.goals[1];
	InitGoal(primary, 2U, SG_STRATEGY_GOAL_CAPTURE_FLAG, INT16_C(10));
	...
	plan.spec.goal_count = 2U;
	plan.binding_count = 3U;
	AddBinding(&plan, 0U, 1U, 1U, 4, preparation_field, fixture,
		&fixture->fields[0]);
	AddBinding(&plan, 1U, 2U, 2U, 5, capture_field, fixture,
		&fixture->fields[1]);
	AddBinding(&plan, 2U, 2U, 3U, 6, fallback_field, fixture,
		&fixture->fields[2]);
	return plan;
}
...
static void TestQueuedPlanAdvancesAcrossGoals(void)
{
	...
	CHECK(output.execution_field == preparation_field);
	CHECK(caller.reducer.plan.goal_count == 2U);
	CHECK(caller.reducer.plan.goals[1].choice_count == 2U);
