...
static void TestStorageAliasingAndCountOverflow(void)
{
	...
	request = PredictionRequest(100U, NULL, prediction_scratch_first,
...
static void TestRuneStorageAliasingRejected(void)
{
	...
	memset(&prediction, 0, sizeof(prediction));
	request = PredictionRequest(100U, NULL, scratch_first,
		scratch_second, 8U,
		(sg_belief_particle_t *)(void *)fixture.model_phases, 1U);
	CHECK(SG_BeliefPredict(&fixture.snapshot, &state, &request, &prediction) ==
		SG_BELIEF_PREDICT_REJECTED_INVALID);
...
static void TestEvidenceAccelerationUsesPhaseAuthority(void)
{
	...
	request = PredictionRequest(200U, NULL, scratch_first,
...
static void ConfigureBeliefMotionPhase(belief_fixture_t *fixture,
	uint32_t phase, sg_belief_motion_state_t movement_state)
{
	sg_rune_phase_basis_t *basis = &fixture->model_phases[phase];

	basis->medium = SG_RUNE_MEDIUM_DRY;
	basis->reference_frame = SG_RUNE_FRAME_WORLD;
	if (movement_state == SG_BELIEF_MOTION_GROUND)
	{
		basis->motion = SG_RUNE_MOTION_SUPPORTED;
		basis->support = SG_RUNE_SUPPORT_SUPPORTED;
	}
	else if (movement_state == SG_BELIEF_MOTION_AIR ||
		 movement_state == SG_BELIEF_MOTION_HOOK)
	{
		basis->motion = SG_RUNE_MOTION_AIRBORNE;
		basis->support = SG_RUNE_SUPPORT_NONE;
	}
	else if (movement_state == SG_BELIEF_MOTION_WATER)
	{
		basis->motion = SG_RUNE_MOTION_SWIMMING;
		basis->support = SG_RUNE_SUPPORT_NONE;
		basis->medium = SG_RUNE_MEDIUM_WATER;
	}
	else
	{
		basis->motion = SG_RUNE_MOTION_SUPPORTED;
		basis->support = SG_RUNE_SUPPORT_MOVER;
		basis->reference_frame = SG_RUNE_FRAME_MOVER_RELATIVE;
	}
}

static void SetBeliefFamilyAcceleration(sg_rune_physics_parameters_t *physics,
	sg_belief_motion_state_t movement_state, float value)
{
	if (movement_state == SG_BELIEF_MOTION_GROUND)
		physics->ground_acceleration = value;
	else if (movement_state == SG_BELIEF_MOTION_AIR)
		physics->air_acceleration = value;
	else if (movement_state == SG_BELIEF_MOTION_WATER)
		physics->water_acceleration = value;
	else if (movement_state == SG_BELIEF_MOTION_HOOK)
		physics->hook_acceleration = value;
	else
		physics->external_acceleration = value;
}

static sg_rune_capability_family_t BeliefCapabilityFamily(
	sg_belief_motion_state_t movement_state)
{
	if (movement_state == SG_BELIEF_MOTION_GROUND)
		return SG_RUNE_CAPABILITY_CONTINUOUS_SUPPORT;
	if (movement_state == SG_BELIEF_MOTION_AIR)
		return SG_RUNE_CAPABILITY_AIRBORNE_CONTROL;
	if (movement_state == SG_BELIEF_MOTION_WATER)
		return SG_RUNE_CAPABILITY_WATER_VOLUME;
	if (movement_state == SG_BELIEF_MOTION_HOOK)
		return SG_RUNE_CAPABILITY_HOOK_TRAJECTORY;
	return SG_RUNE_CAPABILITY_EXTERNAL_FORCE;
}

static void TestVerticalAccelerationBoundariesForEveryFamily(void)
{
	static const sg_belief_motion_state_t movement_states[] = {
	...
	for (index = 0U;
	     index < sizeof(movement_states) / sizeof(movement_states[0]); index++)
	{
		belief_fixture_t fixture;
		sg_belief_evidence_support_t support = Support(0U, 0U, 1.0f);

		BeliefFixtureInit(&fixture);
		ConfigureBeliefMotionPhase(&fixture, 0U, movement_states[index]);
		fixture.model.identity.physics.gravity = 800.0f;
		SetBeliefFamilyAcceleration(&fixture.model.identity.physics,
			movement_states[index], 10.0f);
		support.movement_state = movement_states[index];
		support.acceleration[2] = -810.0f;
		CHECK(SG_BeliefKinematicsCompatible(&fixture.snapshot, &support.phase,
			movement_states[index], support.velocity, support.acceleration,
			support.orientation));
		support.acceleration[2] = nextafterf(-810.0f, -INFINITY);
		CHECK(!SG_BeliefKinematicsCompatible(&fixture.snapshot, &support.phase,
			movement_states[index], support.velocity, support.acceleration,
			support.orientation));
	}
...
static void TestVerticalAccelerationOverflowForEveryFamily(void)
{
	static const sg_belief_motion_state_t movement_states[] = {
		SG_BELIEF_MOTION_GROUND,
		SG_BELIEF_MOTION_AIR,
	...
	for (family = 0U;
	     family < sizeof(movement_states) / sizeof(movement_states[0]); family++)
	{
		belief_fixture_t fixture;
		sg_belief_particle_t storage[8];
		sg_belief_particle_t scratch_first[8];
		...
		BeliefFixtureInit(&fixture);
		ConfigureBeliefMotionPhase(&fixture, 0U, movement_states[family]);
		ConfigureBeliefMotionPhase(&fixture, 1U, movement_states[family]);
		fixture.model.identity.physics.gravity = FLT_MAX;
		SetBeliefFamilyAcceleration(&fixture.model.identity.physics,
			movement_states[family], FLT_MAX);
		fixture.model_kernels[0].family =
			BeliefCapabilityFamily(movement_states[family]);
		fixture.model_kernels[0].parameters.acceleration =
			Interval(0.0f, FLT_MAX);
		fixture.model_kernels[0].parameters.vertical_acceleration =
			Interval(0.0f, FLT_MAX);
		fixture.model_kernels[0].parameters.gravity = FLT_MAX;
		config.policy.diffusion_fraction = 1.0f;
		CHECK(SG_BeliefStateInit(&fixture.snapshot, &state, &config, storage,
			8U));
		support.movement_state = movement_states[family];
		support.acceleration[2] = -FLT_MAX;
		evidence = Evidence(SG_BELIEF_SOURCE_SIGHT, 1U, 100U, &support, 1U);
		frame = Frame(1U, state.revision, 100U, scratch_first,
			scratch_second, 8U);
		frame.evidence = &evidence;
		frame.evidence_count = 1U;
		CHECK(SG_BeliefReduce(&fixture.snapshot, &state, &frame, &reduction) ==
		...
		entries[2] = HorizonEntry(2U, 1U, 2U, 1U, 0.0f, 1.0f);
		step = HorizonCapabilityStep(0U, 0U, 1U, 0U, 0U);
		kernel = HorizonKernel(100U, 200U, entries, 3U, spans, 3U, &step,
			1U);
		frame = Frame(2U, state.revision, 200U, scratch_first,
			scratch_second, 8U);
		frame.kernels = &kernel;
		frame.kernel_count = 1U;
		CHECK(SG_BeliefReduce(&fixture.snapshot, &state, &frame, &reduction) ==
...
static void TestHookVerticalAuthorityMatchesCapabilityStep(void)
{
	belief_fixture_t fixture;
	sg_belief_particle_t storage[8];
	sg_belief_particle_t scratch_first[8];
	...
	BeliefFixtureInit(&fixture);
	ConfigureBeliefMotionPhase(&fixture, 0U, SG_BELIEF_MOTION_HOOK);
	...
	support.movement_state = SG_BELIEF_MOTION_HOOK;
	support.acceleration[2] = -1500.0f;
	evidence = Evidence(SG_BELIEF_SOURCE_SIGHT, 1U, 100U, &support, 1U);
	frame = Frame(1U, state.revision, 100U, scratch_first, scratch_second, 8U);
	frame.evidence = &evidence;
	frame.evidence_count = 1U;
	...
	CHECK(SG_BeliefReduce(&fixture.snapshot, &state, &frame, &reduction) ==
		SG_BELIEF_REDUCE_APPLIED);
	CHECK(state.particle_count == 1U);
	CHECK(state.particles[0].movement_state == SG_BELIEF_MOTION_HOOK);
}
...
static void TestPropagationDecayPredictionAndRuneImmutability(void)
{
	...
	memcpy(particles_before, storage, sizeof(storage));
	memset(&prediction, 0, sizeof(prediction));
	request = PredictionRequest(300U, NULL, scratch_first,
		scratch_second, 8U, predicted, 8U);
	CHECK(SG_BeliefPredict(&fixture.snapshot, &state, &request, &prediction) ==
		SG_BELIEF_PREDICT_APPLIED);
...
static void TestPredictionComposesAcceptedHorizon(void)
{
	belief_fixture_t fixture;
	belief_fixture_t fixture_before;
	sg_belief_particle_t storage[8];
	...
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count, &content_identity));
...
static void TestPredictionAuthorityRejectsFabrication(void)
{
	belief_fixture_t fixture;
	sg_belief_particle_t storage[8];
	sg_belief_particle_t scratch_first[8];
	...
	sg_belief_evidence_t evidence = Evidence(SG_BELIEF_SOURCE_SIGHT, 1U,
		100U, &support, 1U);
	sg_belief_horizon_entry_t complete_entries[4];
	sg_belief_horizon_entry_t omitted_entries[3];
	sg_belief_horizon_entry_t forged_entries[4];
	sg_belief_horizon_span_t complete_spans[3];
	sg_belief_horizon_span_t omitted_spans[3];
	sg_belief_horizon_span_t forged_spans[3];
	sg_belief_horizon_step_t complete_step;
	sg_belief_horizon_step_t forged_step;
	...
	CHECK(SG_BeliefReduce(&fixture.snapshot, &state, &frame, &reduction) ==
		SG_BELIEF_REDUCE_APPLIED);
	complete_entries[0] = HorizonEntry(0U, 0U, 0U, 0U, 0.0f, 0.5f);
	complete_entries[1] = HorizonEntry(0U, 0U, 1U, 0U, 10.0f, 0.5f);
	complete_entries[1].step_count = 1U;
	complete_entries[2] = HorizonEntry(1U, 0U, 1U, 0U, 0.0f, 1.0f);
	complete_entries[3] = HorizonEntry(2U, 1U, 2U, 1U, 0.0f, 1.0f);
	...
	omitted_entries[2] = HorizonEntry(2U, 1U, 2U, 1U, 0.0f, 1.0f);
	omitted_kernel = HorizonKernel(100U, 200U, omitted_entries, 3U,
		omitted_spans, 3U, NULL, 0U);
	CHECK(omitted_kernel.host_complete == 1U);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		&omitted_kernel, 1U, &rejected) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(rejected == NULL);

	memcpy(forged_entries, complete_entries, sizeof(forged_entries));
	forged_entries[0].likelihood = 0.25f;
	forged_entries[1].likelihood = 0.75f;
	forged_step = complete_step;
	...
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &stale_state,
		source, &complete_kernel, 1U, &rejected) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(rejected == NULL);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
	...
	request = PredictionRequest(200U, authority, scratch_first,
		scratch_second, 8U, predicted, 8U);
	CHECK(SG_BeliefPredict(&fixture.snapshot, &stale_state, &request,
		&prediction) == SG_BELIEF_PREDICT_REJECTED_INVALID);
	SG_BeliefTestHorizonAuthorityCorrupt(authority);
	CHECK(SG_BeliefPredict(&fixture.snapshot, &state, &request, &prediction) ==
		SG_BELIEF_PREDICT_REJECTED_INVALID);
	SG_BeliefHorizonAuthorityDestroy(authority);
...
static void TestHorizonOutputAliasBoundaries(void)
{
	...
	memcpy(storage_before, storage, sizeof(storage));
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)(void *)&state.revision) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)(void *)&storage[0]) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)(void *)
			&fixture.snapshot.topology_revision) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)(void *)&fixture.model_kernels[0]) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(memcmp(&fixture, &fixture_before, sizeof(fixture)) == 0);
	CHECK(memcmp(&state, &state_before, sizeof(state)) == 0);
	CHECK(memcmp(storage, storage_before, sizeof(storage)) == 0);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &source) == SG_BELIEF_HORIZON_ACCEPTED);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count, &identity));
	CHECK(kernel_count == 1U);
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		(const sg_belief_horizon_kernel_t **)(void *)&kernel_count,
		&kernel_count, &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count,
		(sg_rune_v2_content_id_t *)(void *)&kernels));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count,
		(sg_rune_v2_content_id_t *)(void *)&kernel_count));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		(const sg_belief_horizon_kernel_t **)(void *)source,
		&kernel_count, &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, (size_t *)(void *)&kernels[0], &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, (size_t *)(void *)kernels[0].origin_spans, &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count,
		(sg_rune_v2_content_id_t *)(void *)kernels[0].entries));
	CHECK(kernels[0].step_count != 0U);
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count,
		(sg_rune_v2_content_id_t *)(void *)kernels[0].steps));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		(const sg_belief_horizon_kernel_t **)(void *)
			&fixture.model_kernels[0],
		&kernel_count, &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, (size_t *)(void *)&fixture.snapshot.topology_revision,
		&identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, (size_t *)(void *)&state.revision, &identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count,
		(sg_rune_v2_content_id_t *)(void *)&storage[0]));
	CHECK(memcmp(&fixture, &fixture_before, sizeof(fixture)) == 0);
	CHECK(memcmp(&state, &state_before, sizeof(state)) == 0);
	CHECK(memcmp(storage, storage_before, sizeof(storage)) == 0);
	SG_BeliefHorizonSourceDestroy(source);
...
static void TestHorizonHandleIdentityPreventsAllocatorAba(void)
{
	belief_fixture_t fixture;
	sg_belief_particle_t storage[8];
	sg_belief_particle_t scratch_first[8];
	...
	sg_belief_particle_t predicted[8];
	sg_belief_state_t state;
	sg_belief_horizon_source_t *stale_source = NULL;
	sg_belief_horizon_source_t *source = NULL;
	sg_belief_horizon_authority_t *stale_authority = NULL;
	sg_belief_horizon_authority_t *authority = NULL;
	const sg_belief_horizon_kernel_t *kernels = NULL;
	const sg_belief_horizon_kernel_t *stale_kernels =
		(const sg_belief_horizon_kernel_t *)(uintptr_t)1U;
	size_t kernel_count = 0U;
	size_t stale_count = 99U;
	sg_rune_v2_content_id_t identity;
	sg_rune_v2_content_id_t stale_identity;
	sg_belief_prediction_request_t request;
	sg_belief_prediction_t prediction;
	sg_belief_prediction_t prediction_before;
	uint64_t stale_source_identity;
	uint64_t source_identity;
	uint64_t stale_authority_identity;
	...
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &stale_source) == SG_BELIEF_HORIZON_ACCEPTED);
	stale_source_identity =
		SG_BeliefTestHorizonSourceIssuanceIdentity(stale_source);
	CHECK(stale_source_identity != 0U);
	SG_BeliefHorizonSourceDestroy(stale_source);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &source) == SG_BELIEF_HORIZON_ACCEPTED);
	source_identity = SG_BeliefTestHorizonSourceIssuanceIdentity(source);
	CHECK(source != stale_source);
	CHECK(source_identity != 0U && source_identity != stale_source_identity);
	memset(&stale_identity, 0xa5, sizeof(stale_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, stale_source,
		&stale_kernels, &stale_count, &stale_identity));
	CHECK(stale_kernels ==
		(const sg_belief_horizon_kernel_t *)(uintptr_t)1U);
	CHECK(stale_count == 99U);
	{
		sg_rune_v2_content_id_t unchanged_identity;

		memset(&unchanged_identity, 0xa5, sizeof(unchanged_identity));
		CHECK(memcmp(&stale_identity, &unchanged_identity,
			sizeof(stale_identity)) == 0);
	}
	SG_BeliefHorizonSourceDestroy(stale_source);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count, &identity));
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count, &stale_authority) ==
		SG_BELIEF_HORIZON_ACCEPTED);
	stale_authority_identity =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(stale_authority);
	SG_BeliefHorizonAuthorityDestroy(stale_authority);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count, &authority) == SG_BELIEF_HORIZON_ACCEPTED);
	authority_identity =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(authority);
	CHECK(authority != stale_authority);
	CHECK(authority_identity != 0U &&
		authority_identity != stale_authority_identity);
	request = PredictionRequest(200U, stale_authority, scratch_first,
		scratch_second, 8U, predicted, 8U);
	memset(&prediction, 0xa5, sizeof(prediction));
	prediction_before = prediction;
	CHECK(SG_BeliefPredict(&fixture.snapshot, &state, &request, &prediction) ==
		SG_BELIEF_PREDICT_REJECTED_INVALID);
	CHECK(memcmp(&prediction, &prediction_before, sizeof(prediction)) == 0);
	SG_BeliefHorizonAuthorityDestroy(stale_authority);
...
static void TestHorizonRegistryAliasIsolation(void)
{
	belief_fixture_t fixture;
	sg_belief_particle_t storage[8];
	sg_belief_particle_t scratch_first[8];
	...
	sg_belief_state_t state;
	sg_belief_horizon_source_t *source = NULL;
	sg_belief_horizon_source_t *other_source = NULL;
	sg_belief_horizon_source_t *retired_source = NULL;
	sg_belief_horizon_authority_t *authority = NULL;
	sg_belief_horizon_authority_t *retired_authority = NULL;
	const sg_belief_horizon_kernel_t *kernels = NULL;
	const sg_belief_horizon_kernel_t *other_kernels = NULL;
	const sg_belief_horizon_kernel_t *view_kernels =
		(const sg_belief_horizon_kernel_t *)(uintptr_t)1U;
	size_t kernel_count = 0U;
	size_t other_kernel_count = 0U;
	size_t view_count = 99U;
	sg_rune_v2_content_id_t identity;
	sg_rune_v2_content_id_t other_identity;
	sg_rune_v2_content_id_t view_identity;
	sg_belief_prediction_request_t request;
	sg_belief_prediction_t prediction;
	uint64_t other_source_issuance;
	uint64_t retired_source_issuance;
	uint64_t authority_issuance;
	uint64_t retired_authority_issuance;
	...
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&kernels, &kernel_count, &identity));
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &other_source) == SG_BELIEF_HORIZON_ACCEPTED);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, other_source,
		&other_kernels, &other_kernel_count, &other_identity));
	other_source_issuance =
		SG_BeliefTestHorizonSourceIssuanceIdentity(other_source);
	CHECK(other_kernel_count == 1U);
	CHECK(other_kernels[0].entry_count != 0U);
	CHECK(other_kernels[0].entries[0].first_step == 0U);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)(void *)
			&other_kernels[0].entries[0].first_step) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);

	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &retired_source) == SG_BELIEF_HORIZON_ACCEPTED);
	retired_source_issuance =
		SG_BeliefTestHorizonSourceIssuanceIdentity(retired_source);
	SG_BeliefHorizonSourceDestroy(retired_source);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)
			SG_BeliefTestHorizonSourcePayloadPointerSlot(retired_source)) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);

	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count, &retired_authority) ==
		SG_BELIEF_HORIZON_ACCEPTED);
	retired_authority_issuance =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(retired_authority);
	SG_BeliefHorizonAuthorityDestroy(retired_authority);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state, 200U,
		(sg_belief_horizon_source_t **)
			SG_BeliefTestHorizonAuthorityPayloadPointerSlot(
				retired_authority)) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	...
	authority_issuance =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(authority);
	memset(&view_identity, 0xa5, sizeof(view_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		(const sg_belief_horizon_kernel_t **)(void *)other_source,
		&view_count, &view_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&view_kernels, (size_t *)(void *)authority, &view_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&view_kernels, &view_count,
		(sg_rune_v2_content_id_t *)(void *)other_kernels[0].entries));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		(const sg_belief_horizon_kernel_t **)(void *)retired_source,
		&view_count, &view_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&view_kernels, (size_t *)(void *)retired_authority,
		&view_identity));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state, source,
		&view_kernels, &view_count,
		(sg_rune_v2_content_id_t *)(void *)retired_source));

	CHECK(SG_BeliefTestHorizonSourceIssuanceIdentity(other_source) ==
		other_source_issuance);
	CHECK(SG_BeliefTestHorizonSourceIssuanceIdentity(retired_source) ==
		retired_source_issuance);
	CHECK(SG_BeliefTestHorizonAuthorityIssuanceIdentity(authority) ==
		authority_issuance);
	CHECK(SG_BeliefTestHorizonAuthorityIssuanceIdentity(retired_authority) ==
		retired_authority_issuance);
	CHECK(SG_BeliefTestHorizonSourceRetired(retired_source));
	CHECK(SG_BeliefTestHorizonAuthorityRetired(retired_authority));
	CHECK(!SG_BeliefHorizonSourceView(&fixture.snapshot, &state,
		retired_source, &view_kernels, &view_count, &view_identity));
	SG_BeliefHorizonSourceDestroy(retired_source);
	SG_BeliefHorizonAuthorityDestroy(retired_authority);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, other_source,
		&other_kernels, &other_kernel_count, &other_identity));
	...
	SG_BeliefHorizonAuthorityDestroy(authority);
	SG_BeliefHorizonSourceDestroy(retired_source);
	SG_BeliefHorizonSourceDestroy(other_source);
	SG_BeliefHorizonSourceDestroy(source);
}

static void TestHorizonAcceptRegistryAliasIsolation(void)
{
	belief_fixture_t fixture;
	sg_belief_particle_t storage[8];
	sg_belief_particle_t scratch_first[8];
	...
	sg_belief_horizon_source_t *source = NULL;
	sg_belief_horizon_source_t *other_source = NULL;
	sg_belief_horizon_source_t *later_source = NULL;
	sg_belief_horizon_authority_t *retired_authority = NULL;
	sg_belief_horizon_authority_t *authority = NULL;
	sg_belief_horizon_authority_t *later_authority = NULL;
	const sg_belief_horizon_kernel_t *retired_kernels = NULL;
	const sg_belief_horizon_kernel_t *kernels = NULL;
	const sg_belief_horizon_kernel_t *other_kernels = NULL;
	const sg_belief_horizon_kernel_t *later_kernels = NULL;
	size_t retired_kernel_count = 0U;
	size_t kernel_count = 0U;
	size_t other_kernel_count = 0U;
	size_t later_kernel_count = 0U;
	size_t *authority_nested_slot;
	sg_rune_v2_content_id_t identity;
	sg_belief_prediction_request_t request;
	sg_belief_prediction_t prediction;
	uint64_t retired_source_issuance;
	uint64_t source_issuance;
	uint64_t other_source_issuance;
	uint64_t retired_authority_issuance;
	uint64_t authority_issuance;
	...
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &retired_source) == SG_BELIEF_HORIZON_ACCEPTED);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state,
		retired_source, &retired_kernels, &retired_kernel_count,
		&identity));
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state,
		retired_source, retired_kernels, retired_kernel_count,
		&retired_authority) == SG_BELIEF_HORIZON_ACCEPTED);
	retired_source_issuance =
		SG_BeliefTestHorizonSourceIssuanceIdentity(retired_source);
	retired_authority_issuance =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(retired_authority);
	...
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count, &authority) == SG_BELIEF_HORIZON_ACCEPTED);
	source_issuance = SG_BeliefTestHorizonSourceIssuanceIdentity(source);
	authority_issuance =
		SG_BeliefTestHorizonAuthorityIssuanceIdentity(authority);
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &other_source) == SG_BELIEF_HORIZON_ACCEPTED);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, other_source,
		&other_kernels, &other_kernel_count, &identity));
	other_source_issuance =
		SG_BeliefTestHorizonSourceIssuanceIdentity(other_source);
	authority_nested_slot = (size_t *)
		SG_BeliefTestHorizonAuthorityFirstEntryStepSlot(authority);
	CHECK(authority_nested_slot != NULL);
	if (authority_nested_slot)
		CHECK(*authority_nested_slot == 0U);
	CHECK(other_kernels[0].entries[0].first_step == 0U);

	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)
			SG_BeliefTestHorizonSourceNextPointerSlot(retired_source)) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)
			SG_BeliefTestHorizonAuthorityNextPointerSlot(
				retired_authority)) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
	...
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)(void *)other_source) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)(void *)authority) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)(void *)
			&other_kernels[0].entries[0].first_step) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state, source,
		kernels, kernel_count,
		(sg_belief_horizon_authority_t **)(void *)authority_nested_slot) ==
		SG_BELIEF_HORIZON_REJECTED_INVALID);

	CHECK(SG_BeliefTestHorizonSourceIssuanceIdentity(retired_source) ==
		retired_source_issuance);
	CHECK(SG_BeliefTestHorizonSourceIssuanceIdentity(source) ==
		source_issuance);
	CHECK(SG_BeliefTestHorizonSourceIssuanceIdentity(other_source) ==
		other_source_issuance);
	CHECK(SG_BeliefTestHorizonAuthorityIssuanceIdentity(retired_authority) ==
	...
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, other_source,
		&other_kernels, &other_kernel_count, &identity));
	CHECK(SG_BeliefHorizonSourceIssue(&fixture.snapshot, &state,
		200U, &later_source) == SG_BELIEF_HORIZON_ACCEPTED);
	CHECK(SG_BeliefHorizonSourceView(&fixture.snapshot, &state, later_source,
		&later_kernels, &later_kernel_count, &identity));
	CHECK(SG_BeliefHorizonAuthorityAccept(&fixture.snapshot, &state,
		later_source, later_kernels, later_kernel_count, &later_authority) ==
		SG_BELIEF_HORIZON_ACCEPTED);
	request = PredictionRequest(200U, later_authority, scratch_first,
		scratch_second, 8U, predicted, 8U);
	CHECK(SG_BeliefPredict(&fixture.snapshot, &state, &request, &prediction) ==
		SG_BELIEF_PREDICT_APPLIED);
	SG_BeliefHorizonSourceDestroy(retired_source);
	SG_BeliefHorizonAuthorityDestroy(retired_authority);
	SG_BeliefHorizonAuthorityDestroy(later_authority);
	SG_BeliefHorizonAuthorityDestroy(authority);
	SG_BeliefHorizonSourceDestroy(later_source);
	SG_BeliefHorizonSourceDestroy(other_source);
	SG_BeliefHorizonSourceDestroy(source);
}
...
int main(void)
{
	...
	TestVerticalAccelerationBoundariesForEveryFamily();
	TestVerticalAccelerationOverflowForEveryFamily();
	TestHookVerticalAuthorityMatchesCapabilityStep();
	...
	TestHorizonAcceptRegistryAliasIsolation();
