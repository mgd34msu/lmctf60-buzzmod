...
// Changes Globals:		-
//===========================================================================
/* The Gladiator game source had this; BuzzMod does not. dm_statusbar and
 * single_statusbar are already globals in g_spawn.c, so only the writer is
 * missing. */
static void SendStatusBar(edict_t *ent, char *string)
{
	if (!ent || !ent->client || !string) return;
