...
#endif
		case MOD_CTF_GRAPPLE: return "Grapple";
...
float BotChatTime(bot_state_t *bs) {
	...
	if (cpm < 1) cpm = 1;	/* the characteristic is clamped to >= 1, but the
				 * divide below is not worth risking on that */

	/* Q3 shipped this hardcoded to 2.0 with the real line commented out. The
	 * botlib does export BotChatLength, so use it: the bot "types" at its own
	 * characters-per-minute rate, and long lines take longer than short ones. */
	return (float) trap_BotChatLength(bs->cs) * 30 / cpm;
}
