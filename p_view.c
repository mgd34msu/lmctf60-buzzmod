...
void P_DamageFeedback (edict_t *player)
{
	...
	if ((level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum))
	{
		...
		if (player->health < 25)
			...
			l = 100;
		gi.sound (player, CHAN_VOICE, gi.soundindex((char *)SG_SexedSound(player, va("pain%i_%i.wav", l, r))), 1, ATTN_NORM, 0);
	}
		...
		{	// drown!
				...
				&& current_player->health > 0)
			{
				...
				if (current_player->health <= current_player->dmg)
					...
					gi.sound (current_player, CHAN_VOICE, gi.soundindex((char *)SG_SexedSound(current_player, "gurp1.wav")), 1, ATTN_NORM, 0);
				else
					gi.sound (current_player, CHAN_VOICE, gi.soundindex((char *)SG_SexedSound(current_player, "gurp2.wav")), 1, ATTN_NORM, 0);
...
void G_SetClientEffects (edict_t *ent)
{
	...
	if (ent->client->quad_framenum > level.framenum)
	{
		...
		{
			// CTF CODE -- LM_JORM			
			if (ent->client->ctf.teamnum == CTF_TEAM_RED) // RED
				ent->s.effects |= EF_PENT;
			else // if (ent->client->teamnum ==2) // Blue
	...
	if (blueflag && blueflag->owner == ent)
...
void Client_Show_High_Scores(edict_t *ent)
{
 char	string[5000];
	...
	strcpy(string, "I am the big dawg");
...
int ClientShowMOD(edict_t *ent)
{
	...
	if (!ent->client->showmod)
		return  1;
	
	time = (level.framenum - ent->client->resp.enterframe);
	if (time < 60) // We just entered the game
	{
		//ent->client->ps.stats[STAT_LAYOUTS] = 1;
		/*
		// Don't show the message if we are moving or pulling up menus
		if (ent->client->showscores || ent->client->showinventory ||
			(ent->client->latched_buttons|ent->client->buttons))
		{
			return 0;
		}
		...
		*/
		...
		switch (ent->client->ctf.teamnum)
		{
			case CTF_TEAM_RED: color = "Red"; break;
			case CTF_TEAM_BLUE: color = "Blue"; break;
...
int ClientShowID(edict_t *ent, char * buf)
{
...
#endif
	char	*color;
	vec3_t	forward, right, start, mins, maxs, end;
	trace_t	tr;
	float	*v;
	edict_t *popup_ent;
	...
	if (ent->client->last_popup_frame &&
		((level.framenum - ent->client->last_popup_frame + ent->client->ctf.original_enterframe) % 10))
		return 0;

	ent->client->last_popup_frame = level.framenum;

	strcpy(string, "");

	// Always show who we are chasing.
#ifdef OLDOBSERVERCODE
	if (ent->client->camera_target)
	{
		ent->client->ctf.popup_ent = ent->client->camera_target;
		ent->client->awayframe = level.framenum;
		sprintf(string2, ":%d", stats_get(ent->client->camera_target, STATS_SCORE));
	}
	else
#endif
	{
		...
		*/
		...
		{
			ent->client->ctf.popup_ent = tr.ent;
			ent->client->awayframe = level.framenum;
		}
		else
		{
	...
	popup_ent = ent->client->ctf.popup_ent;
	if (popup_ent && popup_ent->client)
	{
		ent->client->showctfhud = true;
		color = "string2";

		if (buf != NULL)
			strcpy(buf, popup_ent->client->pers.netname); //surt, for autotext

		strcpy(string, popup_ent->client->pers.netname);
#ifdef OLDOBSERVERCODE
		if (ent->client->camera_target)
			strcat(string, string2);
#endif
		if (popup_ent->client->ctf.teamnum != ent->client->ctf.teamnum)
			color = "string";
	...
	else if (Match_Over())
	{
		ent->client->showctfhud = true;
		color = "string2";
		strcpy(string, "MATCH OVER!");

		Com_sprintf(string, sizeof(string),
			"xv 32 yv 8 picn matchover "
			"xl %i yb %i %s \""
			"%s",
			5,-42,
			color,
	...
	else if (GamePaused())
	{
		ent->client->showctfhud = true;
		color = "string2";
		strcpy(string, "GAME PAUSED (NO MOVING)!");

		Com_sprintf(string, sizeof(string),
			"xv 32 yv 8 picn gamepaused "
			"xl %i yb %i %s \""
			"%s",
			5,-42,
			color,
